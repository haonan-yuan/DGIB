...
sys.path.append(os.path.join(os.path.dirname("__file__"), "..", ".."))
from pytorch_net.util import (
    get_activation,
    get_activation_noise,
    init_weight,
    init_bias,
    init_module_weights,
    init_module_bias,
...
from pytorch_net.util import (
    standardize_symbolic_expression,
    get_param_name_list,
    ...
    if mode == "normal":
        for j in range(output_size):
            ...
            if W_core is not None:
                for i in range(input_size):
                    expression += Symbol("{0}W{1}{2}".format(prefix, i, j)) * Symbol(
                        "x{0}".format(i)
                    )
        ...
        for j in range(output_size):
            ...
            for i in range(input_size):
                expression += (
                    Symbol("x{}".format(i))
                    * Symbol("{0}p{1}".format(prefix, i))
                    * Symbol("{0}q{1}".format(prefix, j))
                )
...
class Simple_Layer(nn.Module):
    def __init__(
        ...
        self.activation = (
            settings["activation"] if "activation" in settings else Default_Activation
        )
        ...
        if "snap_dict" in self.settings:
            ...
            for pos, idx in self.settings["snap_dict"]:
                if (self.weight_on is False and pos == "weight") or (
                    self.bias_on is False and pos == "bias"
                ):
    ...
    def __repr__(self):
        ...
        return 'Simple_Layer({}, "{}"{})'.format(
            self.output_size, self.activation, string
        )
            ...
            if self.weight_on:
                ...
                if new_property == "on":
                    ...
                    self.W_core = nn.Parameter(
                        torch.randn(self.input_size, self.output_size)
                    )
    ...
    def DL(self):
            ...
            for i in range(shape[0]):
                for j in range(shape[1]):
                    if ("weight", (i, j)) in self.snap_dict:
                        snapped_list.append(
                            self.snap_dict[("weight", (i, j))]["new_value"]
                        )
                    else:
        ...
        return get_list_DL(snapped_list, "snapped") + get_list_DL(
            non_snapped_list, "non-snapped"
        )
    ...
    def forward(self, input, p_dict=None):
        ...
        assert output.size(0) == input.size(
            0
        ), "output_size {0} must have same length as input_size {1}. Check shape!".format(
    ...
    def prune_output_neurons(self, neuron_ids):
        ...
        preserved_ids = torch.LongTensor(
            np.array(list(set(range(self.output_size)) - set(neuron_ids)))
        ).to(self.device)
        if self.weight_on:
    ...
    def prune_input_neurons(self, neuron_ids):
        if self.weight_on:
            if not isinstance(neuron_ids, list):
                neuron_ids = [neuron_ids]
            preserved_ids = torch.LongTensor(
                np.array(list(set(range(self.input_size)) - set(neuron_ids)))
            )
            self.W_core = nn.Parameter(self.W_core.data[preserved_ids, :])
    ...
    def add_output_neurons(self, num_neurons, mode="imitation"):
        if mode == "imitation":
            if self.weight_on:
                ...
                new_W_core = (
                    torch.randn(self.input_size, num_neurons) * W_core_std + W_core_mean
                )
            if self.bias_on:
        ...
        if self.weight_on:
            self.W_core = nn.Parameter(
                torch.cat([self.W_core.data, new_W_core.to(self.device)], 1)
            )
        if self.bias_on:
            self.b_core = nn.Parameter(
                torch.cat([self.b_core.data, new_b_core.to(self.device)], 0)
            )
    ...
    def add_input_neurons(self, num_neurons, mode="imitation", position="end"):
        if self.weight_on:
            if mode == "imitation":
                ...
                new_W_core = (
                    torch.randn(num_neurons, self.output_size) * W_core_std
                    + W_core_mean
                )
            elif mode == "zeros":
        ...
        if mode == "snap":
            ...
            if snap_mode == "unsnap":
                ...
                if "snap_targets" in kwargs and kwargs["snap_targets"] is not None:
                    ...
                    excluded_idx_combined = get_idx_list(
                        set(
                            [element[0] for element in excluded_idx]
                            + list(self.snap_dict.keys())
                        ),
                        self.input_size,
                        self.output_size,
                        self.weight_on,
                    ...
                    snap_targets = snap(
                        param,
                        snap_mode=snap_mode,
                        excluded_idx=excluded_idx_combined,
                        top=top,
                    )
                ...
                for idx, new_value in snap_targets:
                    if new_value is not None:
                        if is_target_given:
                            ...
                            pos, true_idx = get_true_idx(
                                idx, self.input_size, self.output_size, self.weight_on
                            )
    ...
    def get_weights_bias(self, is_grad=False):
        if not is_grad:
            W_core = (
                deepcopy(to_np_array(self.W_core, full_reduce=False))
                if self.weight_on
                else None
            )
            b_core = (
                deepcopy(to_np_array(self.b_core, full_reduce=False))
                if self.bias_on
                else None
            )
            ...
            W_grad = (
                deepcopy(to_np_array(W_grad, full_reduce=False))
                if W_grad is not None
                else None
            )
            b_grad = (
                deepcopy(to_np_array(b_grad, full_reduce=False))
    ...
    def get_regularization(self, mode, source=["weight", "bias"]):
        if not isinstance(source, list):
            source = [source]
        reg = Variable(torch.FloatTensor(np.array([0])), requires_grad=False).to(
            self.device
        )
        if self.reg_on:
...
class Utility_Layer(nn.Module):
    def __init__(
        self, input_size, output_size, settings={}, is_cuda=False,
    ):
...
class Symbolic_Layer(nn.Module):
    def __init__(
        ...
        self.output_size = output_size
        self.W_init = (
            W_init  # Here we use W_init to represent all parameter initial values
        )
        self.is_cuda = is_cuda
        self.is_numerical = False
        self.set_symbolic_expression(
            str(settings["symbolic_expression"]), p_init=self.W_init
        )
    ...
    def struct_param(self):
        return [
            self.output_size,
            "Symbolic_Layer",
            {"symbolic_expression": str(self.symbolic_expression)},
        ]
    ...
    def activation(self):
        ...
        for expression in self.symbolic_expression:
            if (
                hasattr(expression.func, "name")
                and expression.func.name in ACTIVATION_LIST
            ):
        ...
        if activation != prev_activation:
            if prev_activation == "linear":
                ...
                new_symbolic_expression = [
                    f(expression) for expression in self.symbolic_expression
                ]
                ...
                if activation != "linear":
                    f = implemented_function(activation, get_activation(activation))
                    new_symbolic_expression = [
                        f(*expression.args) for expression in self.symbolic_expression
                    ]
                else:
                    new_symbolic_expression = [
                        Add(*expression.args) for expression in self.symbolic_expression
                    ]
                self.set_symbolic_expression(new_symbolic_expression)
    ...
    def numerical_expression(self):
        ...
        substitution = [
            (Symbol(param_name), to_np_array(getattr(self, param_name)))
            for param_name in self.param_name_list
        ]
        return [
            expression.subs(substitution) for expression in self.symbolic_expression
        ]
    ...
    def DL(self):
        ...
        expr_length, snapped_list = get_coeffs_tree(
            self.symbolic_expression, param_dict
        )
    ...
    def prune_output_neurons(self, neuron_ids):
        ...
        assert (
            "x" not in variable_names
        ), "In order to prune output_neurons, 'x' cannot be in the symbolic_expression!"
        symbolic_expression = [
            expression
            for i, expression in enumerate(self.symbolic_expression)
            if i not in neuron_ids
        ]
    ...
    def standardize(self, mode="b_mean_zero"):
        ...
        if mode == "b_mean_zero":
            ...
            for expression in self.symbolic_expression:
                ...
                vars_subs = {
                    element: 0 for element in self.get_variable_name_list(expr)
                }
    ...
    def get_param_name_list(self, symbolic_expression=None):
        ...
        symbolic_expression = (
            ...
            if symbolic_expression is None
        ...
        function_name_list = list(
            {
                ...
                for element in expression.atoms(Function)
                if element.func.__name__ not in ["linear"]
            }
        )
        self.implemented_function = {}
        for function_name in function_name_list:
            try:
                self.implemented_function[function_name] = implemented_function(
    ...
    def get_param_dict(self):
        ...
        return {
            param_name: to_np_array(getattr(self, param_name))
    ...
    def get_expression_length(self, symbolic_expression=None):
        ...
        for expression in symbolic_expression:
            ...
            if "x" in variable_list:
                assert (
                    len(variable_list) == 1
                ), "x cannot coexist with x1, x2, etc. in a single expression, since the dimension is not compatible"
    ...
    def set_symbolic_expression(self, symbolic_expression, p_init=None):
        ...
        symbolic_expression = standardize_symbolic_expression(symbolic_expression)
        assert (
            sum(self.get_expression_length(symbolic_expression)) == self.output_size
        ), "symbolic_expression's combined output length must be equal to self.output_size!"
        self.old_param_name_list = (
            self.get_param_name_list(self.symbolic_expression)
            ...
            if not hasattr(self, param_name):
                ...
                if param_init is None:
                    ...
                    setattr(
                        self,
                        param_name,
                        nn.Parameter(torch.FloatTensor(np.array([param_init]))),
                    )
    ...
    def forward(self, input, p_dict=None):
        ...
        if p_dict is None:
            symbols = tuple(
                symbols + [Symbol(param_name) for param_name in self.param_name_list]
            ...
            symbols = tuple(
                ...
                + [
                    Symbol(param_name)
                    for param_name in sorted(list(p_dict.keys()))
                    if "x" not in param_name
                ]
            )
        f_list = [
            lambdify(symbols, N(expression), torch)
            for expression in self.symbolic_expression
        ]  # Obtain the lambda function f(x0, x1,..., param0, param1, ...)
        ...
        if p_dict is None:
            symbols_feed = variables_feed + [
                getattr(self, param_name) for param_name in self.param_name_list
            ]
        else:
            symbols_feed = variables_feed + [
                p_dict[param_name]
        ...
        for f in f_list:
            ...
            if not isinstance(output_ele, Variable):
                output_ele = (
                    to_Variable(torch.ones(input.shape[0], 1), is_cuda=self.is_cuda)
                    * output_ele
                )
            elif len(output_ele.shape) < 2 or output_ele.shape[0] != input.shape[0]:
                multiplier = to_Variable(
                    torch.ones(input.shape[0], 1), is_cuda=self.is_cuda
                )
    ...
    def simplify(self, mode="form", **kwargs):
        ...
        for mode_ele in mode:
            if mode_ele == "form":
                ...
                new_expression = [
                    simplify(expression) for expression in self.symbolic_expression
                ]
                self.set_symbolic_expression(new_expression)
                if verbose > 0:
                    print(
                        "Original expression:\tsymbolic: {0}; \t numerical: {1}".format(
                            prev_expression, self.numerical_expression
                        )
                    )
                    print(
                        "New  expression: \tsymbolic: {0}; \t numerical: {1}".format(
                            self.symbolic_expression, self.numerical_expression
                        )
                    )
            elif mode_ele == "snap":
                snap_mode = kwargs["snap_mode"] if "snap_mode" in kwargs else "integer"
                ...
                if snap_mode == "unsnap":
                    unsnapped_expression, new_param_dict = unsnap(
                        self.symbolic_expression, self.get_param_dict()
                    )
                    self.set_symbolic_expression(unsnapped_expression, new_param_dict)
                    info_list = info_list + [
                        (mode_ele, snap_mode, unsnapped_expression)
                    ]
                elif snap_mode in ["vector"]:
                    param_dict = self.get_param_dict()
                    param_dict_subs, new_param_dict = snap(
                        param_dict, snap_mode=snap_mode, top=top
                    )
                    ...
                    if verbose > 0:
                        ...
                        print(
                            "Substitution:  \t{0}, with value: {1}".format(
                                pprint_dict(param_dict_subs),
                                pprint_dict(new_param_dict),
                            )
                        )
                    new_expression = [
                        expression.subs(param_dict_subs)
                        for expression in self.symbolic_expression
                    ]
                    self.set_symbolic_expression(new_expression, param_dict)
                    info_list = info_list + [
                        (deepcopy(param_dict_subs), deepcopy(new_param_dict))
                    ]
                    if verbose > 0:
                        print(
                            "New  expression: \tsymbolic: {0}; \t numerical: {1}".format(
                    ...
                    if not (len(snap_targets) == 1 and snap_targets[0][1] is None):
                        subs_targets = [
                            (Symbol(param_names[idx]), new_value)
                            for idx, new_value in snap_targets
                        ]
                        prev_expression = self.symbolic_expression
                        if verbose > 0:
                ...
                if len(self.get_param_dict()) < 2:
                    ...
                    snap_mode = (
                        kwargs["snap_mode"] if "snap_mode" in kwargs else "integer"
                    )
                    top = kwargs["top"] if "top" in kwargs else 1
                    if snap_mode == "integer":
                        ...
                        raise Exception(
                            "snap_mode {} not recognized!".format(snap_mode)
                        )
                    param_list, inverse_dict = get_param_inverse_dict(
                        self.get_param_dict()
                    )
                    ...
                    subs_targets = [
                        (
                            Symbol(inverse_dict[replace_id]),
                            Symbol(inverse_dict[ref_id]) * ratio,
                        )
                        for (replace_id, ref_id), ratio in snap_targets
                    ]
                    prev_expression = self.symbolic_expression
                    new_expression = [
                        expression.subs(subs_targets)
                    ...
                    info_list = info_list + [
                        (
                            inverse_dict[replace_id],
                            "{} * ".format(ratio) + inverse_dict[ref_id],
                        )
                        for (replace_id, ref_id), ratio in snap_targets
                    ]
                    if verbose > 0:
                        print(
                            "Original expression:\tsymbolic: {}; \t numerical: {}".format(
                                prev_expression, self.numerical_expression
                            )
                        )
                        print("Substitution:  \t{}".format(subs_targets))
                        print(
                            "New  expression: \tsymbolic: {}; \t numerical: {}".format(
                                self.symbolic_expression, self.numerical_expression
                            )
                        )
            else:
    ...
    def get_regularization(self, mode, source=["weight"], **kwargs):
        ...
        if len(param_list) > 0 and "weight" in source:
            ...
            scale_factor = (
                kwargs["reg_scale_factor"] if "reg_scale_factor" in kwargs else None
            )
            if mode == "L1":
                if scale_factor is not None:
                    reg_indi = (
                        (params * to_Variable(scale_factor, is_cuda=self.is_cuda))
                        .abs()
                        .sum()
                    )
                else:
                ...
                if scale_factor is not None:
                    reg_indi = torch.sum(
                        (params * to_Variable(scale_factor, is_cuda=self.is_cuda)) ** 2
                    )
                else:
...
class SuperNet_Layer(nn.Module):
    def __init__(
        ...
        self.W_available = (
            settings["W_available"]
            if "W_available" in settings
            else ["dense", "Toeplitz"]
        )
        self.b_available = (
            settings["b_available"] if "b_available" in settings else ["dense", "None"]
        )
        self.A_available = (
            settings["A_available"] if "A_available" in settings else ["linear", "relu"]
        )
        self.W_sig_init = (
            settings["W_sig_init"] if "W_sig_init" in settings else None
        )  # initialization for the significance for the weights
        self.b_sig_init = (
            settings["b_sig_init"] if "b_sig_init" in settings else None
        )  # initialization for the significance for the bias
        self.A_sig_init = (
            settings["A_sig_init"] if "A_sig_init" in settings else None
        )  # initialization for the significance for the activations
    ...
    def init_layer(self):
        self.W_layer_seed = nn.Parameter(
            torch.FloatTensor(np.random.randn(self.input_size, self.output_size))
        )
        ...
        if "arithmetic-series-in" in self.W_available:
            self.W_interval_j = nn.Parameter(
                torch.randn(self.output_size)
                / np.sqrt(self.input_size + self.output_size)
            )
        if "arithmetic-series-out" in self.W_available:
            self.W_interval_i = nn.Parameter(
                torch.randn(self.input_size)
                / np.sqrt(self.input_size + self.output_size)
            )
        if "arithmetic-series-2D-in" in self.W_available:
            self.W_mean_2D_in = nn.Parameter(
                torch.randn(self.output_size)
                / np.sqrt(
                    self.input_size_2D[0] + self.input_size_2D[1] + self.output_size
                )
            )
            self.W_interval_2D_in = nn.Parameter(
                torch.randn(2, self.output_size)
                / np.sqrt(
                    self.input_size_2D[0] + self.input_size_2D[1] + self.output_size
                )
            )
        if "arithmetic-series-2D-out" in self.W_available:
            self.W_mean_2D_out = nn.Parameter(
                torch.randn(self.input_size)
                / np.sqrt(
                    self.input_size + self.output_size_2D[0] + self.output_size_2D[1]
                )
            )
            self.W_interval_2D_out = nn.Parameter(
                torch.randn(2, self.input_size)
                / np.sqrt(
                    self.input_size + self.output_size_2D[0] + self.output_size_2D[1]
                )
            )
        ...
        if "arithmetic-series-2D" in self.b_available:
            ...
            self.b_interval_2D = nn.Parameter(
                torch.randn(2)
                / np.sqrt(self.output_size_2D[0] + self.output_size_2D[1])
            )
    ...
    def get_layers(self, source=["weight", "bias"]):
        ...
        if "weight" in source:
            ...
            for weight_type in self.W_available:
                if weight_type == "dense":
                    ...
                    if self.output_size > 1:
                        inv_idx = (
                            torch.arange(self.output_size - 1, 0, -1)
                            .long()
                            .to(self.device)
                        )
                        W_seed = torch.cat(
                            [self.W_layer_seed[0][inv_idx], self.W_layer_seed[:, 0]]
                        )
                    else:
                        W_seed = self.W_layer_seed[:, 0]
                    for j in range(self.output_size):
                        W_layer_stacked.append(
                            W_seed[
                                self.output_size
                                - j
                                - 1 : self.output_size
                    ...
                    idx_i = torch.FloatTensor(
                        np.repeat(np.arange(self.input_size), self.output_size)
                    ).to(self.device)
                    idx_j = torch.LongTensor(
                        range(self.output_size) * self.input_size
                    ).to(self.device)
                    offset = self.input_size / float(2) - 0.5
                    W_layer = (
                        mean_j[idx_j]
                        + self.W_interval_j[idx_j]
                        * Variable(idx_i - offset, requires_grad=False)
                    ).view(self.input_size, self.output_size)
                elif weight_type == "arithmetic-series-out":
                    mean_i = self.W_layer_seed.mean(1)
                    idx_i = torch.LongTensor(
                        np.repeat(np.arange(self.input_size), self.output_size)
                    ).to(self.device)
                    idx_j = torch.FloatTensor(
                        range(self.output_size) * self.input_size
                    ).to(self.device)
                    offset = self.output_size / float(2) - 0.5
                    W_layer = (
                        mean_i[idx_i]
                        + self.W_interval_i[idx_i]
                        * Variable(idx_j - offset, requires_grad=False)
                    ).view(self.input_size, self.output_size)
                elif weight_type == "arithmetic-series-2D-in":
                    idx_i, idx_j, idx_k = np.meshgrid(
                        range(self.input_size_2D[0]),
                        range(self.input_size_2D[1]),
                    ...
                    W_layer = (
                        self.W_mean_2D_in[idx_k]
                        + self.W_interval_2D_in[:, idx_k][0]
                        * Variable(idx_i - offset_i, requires_grad=False)
                        + self.W_interval_2D_in[:, idx_k][1]
                        * Variable(idx_j - offset_j, requires_grad=False)
                    ).view(self.input_size, self.output_size)
                elif weight_type == "arithmetic-series-2D-out":
                    idx_k, idx_i, idx_j = np.meshgrid(
                        range(self.input_size),
                        range(self.output_size_2D[0]),
                        range(self.output_size_2D[1]),
                        indexing="ij",
                    )
                    idx_k = torch.from_numpy(idx_k).long().view(-1).to(self.device)
                    idx_i = torch.from_numpy(idx_i).float().view(-1).to(self.device)
                    ...
                    W_layer = (
                        self.W_mean_2D_out[idx_k]
                        + self.W_interval_2D_out[:, idx_k][0]
                        * Variable(idx_i - offset_i, requires_grad=False)
                        + self.W_interval_2D_out[:, idx_k][1]
                        * Variable(idx_j - offset_j, requires_grad=False)
                    ).view(self.input_size, self.output_size)
                else:
                    raise Exception(
                        "weight_type '{0}' not recognized!".format(weight_type)
                    )
            ...
            if len(self.W_available) == 1:
                ...
                self.W_core = torch.matmul(
                    self.W_list, W_sig_softmax.transpose(1, 0)
                ).squeeze(2)
        ...
        if "bias" in source:
            ...
            for bias_type in self.b_available:
                if bias_type == "None":
                    b_layer = Variable(
                        torch.zeros(self.output_size).to(self.device),
                        requires_grad=False,
                    )
                    ...
                    offset = self.output_size / float(2) - 0.5
                    idx = Variable(
                        torch.FloatTensor(range(self.output_size)).to(self.device),
                        requires_grad=False,
                    )
                    ...
                    b_layer = (
                        self.b_mean_2D
                        + self.b_interval_2D[0]
                        * Variable(idx_i - offset_i, requires_grad=False)
                        + self.b_interval_2D[1]
                        * Variable(idx_j - offset_j, requires_grad=False)
                    ).view(-1)
            ...
            if len(self.b_available) == 1:
                ...
                self.b_core = torch.matmul(
                    self.b_list, b_sig_softmax.transpose(1, 0)
                ).squeeze(1)
