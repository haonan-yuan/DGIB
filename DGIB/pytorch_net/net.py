...
from DGIB.pytorch_net.modules import get_Layer, load_layer_dict, Simple_2_Symbolic
...
def train(
    ...
    **kwargs
            ...
            for reg_type, reg_coeff in reg_dict.items():
                ...
                reg = (
                    ...
                    + model.get_regularization(
                        source=[reg_type], mode=reg_mode, **kwargs
                    )
                    * reg_coeff_ele
                )
        return reg
    ...
    scheduler_type = (
        kwargs["scheduler_type"] if "scheduler_type" in kwargs else "ReduceLROnPlateau"
    )
    gradient_noise = kwargs["gradient_noise"] if "gradient_noise" in kwargs else None
    data_loader_apply = (
        kwargs["data_loader_apply"] if "data_loader_apply" in kwargs else None
    )

    # Inspection kwargs:
    inspect_step = (
        kwargs["inspect_step"] if "inspect_step" in kwargs else None
    ...
    inspect_functions = (
        kwargs["inspect_functions"] if "inspect_functions" in kwargs else None
    )
    ...
    inspect_items_interval = (
        kwargs["inspect_items_interval"] if "inspect_items_interval" in kwargs else 1000
    )
    inspect_image_interval = (
        kwargs["inspect_image_interval"] if "inspect_image_interval" in kwargs else None
    )
    inspect_loss_precision = (
        kwargs["inspect_loss_precision"] if "inspect_loss_precision" in kwargs else 4
    )
    ...
    if patience is not None:
        early_stopping_epsilon = (
            kwargs["early_stopping_epsilon"]
            if "early_stopping_epsilon" in kwargs
            else 0
        )
        early_stopping_monitor = (
            kwargs["early_stopping_monitor"]
        ...
        early_stopping = Early_Stopping(
            ...
            mode="max" if early_stopping_monitor in ["accuracy"] else "min",
        )
    ...
    label_noise_matrix = (
        kwargs["label_noise_matrix"] if "label_noise_matrix" in kwargs else None
    )
    transform_label = Transform_Label(
        label_noise_matrix=label_noise_matrix, is_cuda=is_cuda
    )
    ...
    if co_kwargs is not None:
        ...
        co_criterion = (
            co_kwargs["co_criterion"] if "co_criterion" in co_kwargs else None
        )
        co_multi_step = (
            co_kwargs["co_multi_step"] if "co_multi_step" in co_kwargs else 1
        )

    # Get original loss:
    if len(inspect_items_train) > 0:
        loss_value_train = get_loss(
            model,
            train_loader,
            X,
            y,
        ...
        info_dict_train = prepare_inspection(
            model, train_loader, X, y, transform_label=transform_label, **kwargs
        )
        if "loss" in record_keys:
            record_data(data_record, [loss_value_train], ["loss_tr"])
    loss_original = get_loss(
        model,
        validation_loader,
        X_valid,
        y_valid,
    ...
    if "param_grad" in record_keys:
        record_data(
            data_record,
            [model.get_weights_bias(W_source="core", b_source="core", is_grad=True)],
            ["param_grad"],
        )
    if co_kwargs is not None:
        co_loss_original = get_loss(
            co_model,
            validation_loader,
            X_valid,
            y_valid,
    ...
    optimizer = (
        get_optimizer(optim_type, lr, parameters, **optim_kwargs)
        if "optimizer" not in kwargs
        or ("optimizer" in kwargs and kwargs["optimizer"] is None)
    ...
    if inspect_items is not None:
        print("{}:".format(-1), end="")
        print(
            "\tlr: {0:.3e}\t loss:{1:.{2}f}".format(
                optimizer.param_groups[0]["lr"], loss_original, inspect_loss_precision
            ),
            end="",
        )
        info_dict = prepare_inspection(
            model,
            validation_loader,
            X_valid,
            y_valid,
        ...
        if len(inspect_items_train) > 0:
            print(
                "\tloss_tr: {0:.{1}f}".format(loss_value_train, inspect_loss_precision),
                end="",
            )
        ...
        if (
            "reg" in record_keys
            and "reg_dict" in kwargs
            and len(kwargs["reg_dict"]) > 0
        ):
            print(
                "\treg:{0:.{1}f}".format(
                    to_np_array(reg_value), inspect_loss_precision
                ),
                end="",
            )
        if len(info_dict) > 0:
            for item in inspect_items:
                if item in info_dict:
                    print(
                        " \t{0}: {1:.{2}f}".format(
                            item, info_dict[item], inspect_loss_precision
                        ),
                        end="",
                    )
        ...
        if co_kwargs is not None:
            co_info_dict = prepare_inspection(
                co_model,
                validation_loader,
                X_valid,
                y_valid,
            ...
            if "co_loss" in inspect_items:
                ...
                print(
                    "\tco_loss: {}".format(
                        formalize_value(co_loss_value, inspect_loss_precision)
                    ),
                    end="",
                )
                    ...
                    if item in co_info_dict:
                        print(
                            " \t{0}: {1}".format(
                                item,
                                formalize_value(
                                    co_info_dict[item], inspect_loss_precision
                                ),
                            ),
                            end="",
                        )
                        if item in record_keys and item != "loss":
                            record_data(
                                data_record, [to_np_array(co_info_dict[item])], [item]
                            )
        print("\n")
    ...
    if gradient_noise is not None:
        ...
        scale_gen = Gradient_Noise_Scale_Gen(
            epochs=epochs,
    ...
    if scheduler_type is not None:
        if scheduler_type == "ReduceLROnPlateau":
            scheduler_patience = (
                kwargs["scheduler_patience"] if "scheduler_patience" in kwargs else 40
            )
            scheduler_factor = (
                kwargs["scheduler_factor"] if "scheduler_factor" in kwargs else 0.1
            )
            scheduler_verbose = (
                kwargs["scheduler_verbose"] if "scheduler_verbose" in kwargs else False
            )
            ...
            scheduler_lr_lambda = (
                kwargs["scheduler_lr_lambda"]
                if "scheduler_lr_lambda" in kwargs
                else (lambda epoch: 0.97 ** (epoch // 2))
            )
    ...
    for i in range(epochs + 1):
                            ...
                            h = param.register_hook(
                                ...
                                + Variable(
                                    torch.normal(
                                        ...
                                        std=current_gradient_noise_scale
                                        * torch.ones(grad.size()),
                                    )
                                )
                            )
        ...
        if X is not None and y is not None:
            ...
            if co_kwargs is not None:
                if (
                    "co_warmup_epochs" not in co_kwargs
                    or "co_warmup_epochs" in co_kwargs
                    and i >= co_kwargs["co_warmup_epochs"]
                ):
            ...
            for k, data_batch in enumerate(train_loader):
                        ...
                        if len(model.info_dict) > 0:
                            for item in inspect_items:
                                if item in model.info_dict:
                                    logger.log_scalar(
                                        item, model.info_dict[item], batch_idx
                                    )
                    optimizer.step()
                else:
                ...
                # Rampup scheduler:
                if (
                    lr_rampup_steps is not None
                    and i * data_size // len(X_batch) + k < lr_rampup_steps
                ):
                ...
                if co_kwargs is not None:
                    if (
                        ...
                        for _ in range(co_multi_step):
                            co_optimizer.zero_grad()
                            co_reg = get_regularization(
                                co_model, loss_epoch=i, **co_kwargs
                            )
                            co_loss = (
                                co_model.get_loss(
                                    X_batch,
                ...
                if inspect_step is not None:
                    if k % inspect_step == 0:
                        ...
                        if "loss" in inspect_items:
                            ...
                            print(
                                "\tloss: {0:.{1}f}".format(
                                    loss.item(), inspect_loss_precision
                                ),
                                end="",
                            )
                        if len(info_dict) > 0:
                                ...
                                if item in info_dict:
                                    ...
                                    print(
                                        " \t{0}: {1}".format(
                                            item,
                                            formalize_value(
                                                info_dict[item], inspect_loss_precision
                                            ),
                                        ),
                                        end="",
                                    )
                        if co_kwargs is not None:
                            if (
                                ...
                                if "co_loss" in inspect_items:
                                    print(
                                        "\tco_loss: {0:.{1}f}".format(
                                            co_loss.item(), inspect_loss_precision
                                        ),
                                        end="",
                                    )
                    ...
                    if k % save_step == 0:
                        if filename is not None:
                            pickle.dump(
                                model.model_dict, open(filename[:-2] + "_model.p", "wb")
                            )
        ...
        if i % inspect_interval == 0:
            model.eval()
            if (
                inspect_items is not None
                and i % inspect_items_interval == 0
                and len(inspect_items_train) > 0
            ):
                loss_value_train = get_loss(
                    model,
                    train_loader,
            ...
            if callback is not None:
                ...
                callback(
                    model=model, X=X_valid, y=y_valid, iteration=i, loss=loss_value,
                )
            if patience is not None:
            ...
            if inspect_items is not None:
                if i % inspect_items_interval == 0:
                    ...
                    if len(inspect_items_train) > 0:
                        print(
                            "\tloss_tr: {0:.{1}f}".format(
                                loss_value_train, inspect_loss_precision
                            ),
                            end="",
                        )
                        info_dict_train = update_key_train(
                            info_dict_train, inspect_items_train
                        )
                        info_dict.update(info_dict_train)
                    if (
                        "reg" in inspect_items
                        and "reg_dict" in kwargs
                        and len(kwargs["reg_dict"]) > 0
                    ):
                        print(
                    ...
                    if len(info_dict) > 0:
                        for item in inspect_items:
                            if item + "_val" in info_dict:
                                ...
                                if item in record_keys and item not in ["loss", "reg"]:
                                    record_data(
                                        data_record,
                                        [to_np_array(info_dict[item + "_val"])],
                                        [item],
                                    )
                        ...
                        if logdir is not None:
                            for item in inspect_items:
                                if item + "_val" in info_dict:
                                    logger.log_scalar(
                                        item + "_val", info_dict[item + "_val"], i
                                    )
                    ...
                    if co_kwargs is not None:
                        ...
                        if "co_loss" in inspect_items:
                            print(
                                "\tco_loss: {0:.{1}f}".format(
                                    co_loss_value, inspect_loss_precision
                                ),
                                end="",
                            )
                        if len(co_info_dict) > 0:
                            for item in inspect_items:
                                if item + "_val" in co_info_dict:
                                    ...
                                    if item in record_keys and item != "co_loss":
                                        record_data(
                                            data_record,
                                            [to_np_array(co_info_dict[item + "_val"])],
                                            [item],
                                        )
                        if "co_loss" in record_keys:
                            record_data(data_record, [co_loss_value], ["co_loss"])
            ...
            if isplot:
                if inspect_image_interval is not None and hasattr(model, "plot"):
                    if i % inspect_image_interval == 0:
                        if gradient_noise is not None:
                            print(
                                "gradient_noise: {0:.9f}".format(
                                    current_gradient_noise_scale
                                )
                            )
                        plot_model(
                            model,
                            ...
                            y=y_valid,
                            transform_label=transform_label,
                            data_loader_apply=data_loader_apply,
                        )
                if (
                    co_kwargs is not None
                    and "inspect_image_interval" in co_kwargs
                    and co_kwargs["inspect_image_interval"]
                    and hasattr(co_model, "plot")
                ):
...
def train_simple(model, X, y, validation_data=None, inspect_interval=5, **kwargs):
        ...
        for model_ele in model:
            if reg_dict is not None:
                for reg_type, reg_coeff in reg_dict.items():
                    reg = (
                        reg
                        + model_ele.get_regularization(
                            source=[reg_type], mode="L1", **kwargs
                        )
                        * reg_coeff
                    )
        return reg
    ...
    early_stopping_epsilon = (
        kwargs["early_stopping_epsilon"] if "early_stopping_epsilon" in kwargs else 0
    )
    patience = kwargs["patience"] if "patience" in kwargs else 40
    record_keys = (
        kwargs["record_keys"]
        if "record_keys" in kwargs
        else ["loss", "mse", "data_DL", "model_DL"]
    )
    scheduler_type = (
        kwargs["scheduler_type"] if "scheduler_type" in kwargs else "ReduceLROnPlateau"
    )
    loss_precision_floor = (
        kwargs["loss_precision_floor"]
        if "loss_precision_floor" in kwargs
        else PrecisionFloorLoss
    )
    ...
    if patience is not None:
        early_stopping = Early_Stopping(
            patience=patience, epsilon=early_stopping_epsilon
        )
    ...
    DL_criterion = Loss_Fun(
        core="DLs", loss_precision_floor=loss_precision_floor, DL_sum=True
    )
    DL_criterion_absolute = Loss_Fun(
        core="DLs", loss_precision_floor=PrecisionFloorLoss, DL_sum=True
    )
    ...
    if "mse" in record_keys:
        record_data(
            data_record, [to_np_array(nn.MSELoss()(pred_valid, y_valid))], ["mse"]
        )
    if "data_DL" in record_keys:
        record_data(
            data_record, [to_np_array(DL_criterion(pred_valid, y_valid))], ["data_DL"]
        )
    if "data_DL_absolute" in record_keys:
        record_data(
            data_record,
            [to_np_array(DL_criterion_absolute(pred_valid, y_valid))],
            ["data_DL_absolute"],
        )
    ...
    if "param" in record_keys:
        record_data(
            data_record,
            [model[0].get_weights_bias(W_source="core", b_source="core")],
            ["param"],
        )
    if "param_grad" in record_keys:
        record_data(
            data_record,
            [model[0].get_weights_bias(W_source="core", b_source="core", is_grad=True)],
            ["param_grad"],
        )
    if "param_collapse_layers" in record_keys:
        record_data(
            data_record,
            [
                simplify(deepcopy(model[0]), X, y, "collapse_layers", verbose=0)[
                    0
                ].get_weights_bias(W_source="core", b_source="core")
            ],
            ["param"],
        )
    ...
    if num_params == 0:
        ...
        if "data_DL" in record_keys:
            record_data(
                data_record,
                [to_np_array(DL_criterion(pred_valid, y_valid))],
                ["data_DL"],
            )
        if "data_DL_absolute" in record_keys:
            record_data(
    ...
    if scheduler_type is not None:
        if scheduler_type == "ReduceLROnPlateau":
            scheduler_patience = (
                kwargs["scheduler_patience"] if "scheduler_patience" in kwargs else 10
            )
            scheduler_factor = (
                kwargs["scheduler_factor"] if "scheduler_factor" in kwargs else 0.1
            )
            ...
            scheduler_lr_lambda = (
                kwargs["scheduler_lr_lambda"]
                if "scheduler_lr_lambda" in kwargs
                else (lambda epoch: 1 / (1 + 0.01 * epoch))
            )
            scheduler = LambdaLR(optimizer, lr_lambda=scheduler_lr_lambda)
        else:
            raise
    ...
    for i in range(epochs + 1):
        ...
        if i % inspect_interval == 0:
            ...
            if "mse" in record_keys:
                record_data(
                    data_record,
                    [to_np_array(nn.MSELoss()(pred_valid, y_valid))],
                    ["mse"],
                )
            if "data_DL" in record_keys:
                record_data(
            ...
            if "param_collapse_layers" in record_keys:
                record_data(
                    ...
                    [
                        simplify(
                            deepcopy(model[0]), X, y, "collapse_layers", verbose=0
                        )[0].get_weights_bias(W_source="core", b_source="core")
                    ],
                    ["param"],
                )
            if patience is not None:
...
def load_model_dict_net(model_dict, is_cuda=False):
    ...
    if net_type.startswith("MLP"):
        ...
        return Branching_Net(
            net_base_model_dict=model_dict["net_base_model_dict"],
        ...
        return Fan_in_MLP(
            model_dict_branch1=model_dict["model_dict_branch1"],
        ...
        return ConvNet(
            ...
            if "return_indices" in model_dict
            else False,
            is_cuda=is_cuda,
        )
    elif net_type == "Conv_Autoencoder":
        model = Conv_Autoencoder(
            input_channels_encoder=model_dict["input_channels_encoder"],
        ...
        is_generative = (
            model_dict["is_generative"] if "is_generative" in model_dict else False
        )
        return Conv_Model(
            encoder_model_dict=model_dict["encoder_model_dict"]
...
def get_loss(
    ...
    **kwargs
    ...
    max_validation_iter = (
        kwargs["max_validation_iter"] if "max_validation_iter" in kwargs else None
    )
    ...
    if data_loader is not None:
        ...
        for j, data_batch in enumerate(data_loader):
            if isinstance(data_batch, tuple) or isinstance(data_batch, list):
                X_batch, y_batch = data_batch
                if (
                    "data_loader_apply" in kwargs
                    and kwargs["data_loader_apply"] is not None
                ):
                    X_batch, y_batch = kwargs["data_loader_apply"](X_batch, y_batch)
            else:
                X_batch, y_batch = kwargs["data_loader_apply"](data_batch)
            loss_ele = to_np_array(
                model.get_loss(
                    X_batch, transform_label(y_batch), criterion=criterion, **kwargs
                )
            )
            if j == 0:
        ...
        loss = to_np_array(
            model.get_loss(X, transform_label(y), criterion=criterion, **kwargs)
        )
    return loss
...
def prepare_inspection(
    model, data_loader=None, X=None, y=None, transform_label=None, **kwargs
):
    inspect_functions = (
        kwargs["inspect_functions"] if "inspect_functions" in kwargs else None
    )
    ...
    if data_loader is None:
        ...
        if inspect_functions is not None:
            for inspect_function_key, inspect_function in inspect_functions.items():
                all_dict_summary[inspect_function_key] = inspect_function(
                    model, X, y, **kwargs
                )
    else:
        assert X is None and y is None
        ...
        for j, data_batch in enumerate(data_loader):
            ...
            if isinstance(data_batch, tuple) or isinstance(data_batch, list):
                ...
                X_batch, y_batch = kwargs["data_loader_apply"](data_batch)
            info_dict = model.prepare_inspection(
                X_batch, transform_label(y_batch), valid_step=j, **kwargs
            )
            ...
            if inspect_functions is not None:
                for inspect_function_key, inspect_function in inspect_functions.items():
                    inspect_function_result = inspect_function(
                        model, X_batch, transform_label(y_batch), **kwargs
                    )
...
def simplify(
    ...
    **kwargs
    ...
    if validation_data is None:
        ...
        X_valid, y_valid = validation_data
    simplify_criteria = (
        kwargs["simplify_criteria"]
        if "simplify_criteria" in kwargs
        else ("DLs", 0.05, 3, "relative")
    )  # the first argument choose from "DL", "loss"
    ...
    performance_monitor = Performance_Monitor(
        patience=simplify_patience,
        epsilon=simplify_epsilon,
        compare_mode=simplify_compare_mode,
    )
        ...
        if mode_ele == "collapse_layers":
            ...
            for model_id, model_ele in enumerate(model):
                ...
                for k in range(len(model_ele.struct_param)):
                    if "activation" in model_ele.struct_param[k][2]:
                        activation_list.append(
                            model_ele.struct_param[k][2]["activation"]
                        )
                ...
                for k, activation in enumerate(activation_list):
                    if activation == "linear" and k != len(activation_list) - 1:
                        if k not in collapse_dict and current_start is None:
                            # Create a new bunch:
                            if (
                                model_ele.struct_param[k + 1][1]
                                == model_ele.struct_param[k][1]
                            ):  # The current layer must have the same layer_type as the next layer
                                current_start = k
                ...
                for current_start, layer_ids in collapse_dict.items():
                    ...
                    new_layer_info[current_start] = {
                        "W_init": W_accum.data.numpy(),
                        "b_init": b_accum.data.numpy(),
                        "layer_struct_param": [
                            b_accum.size(0),
                            model_ele.struct_param[last_layer_id][1],
                            deepcopy(model_ele.struct_param[last_layer_id][2]),
                        ],
                    }
                    new_layer_info[current_start].pop("snap_dict", None)
                if verbose >= 1:
                    print(
                        "model_id {}, layers collapsed: {}".format(
                            model_id, collapse_dict
                        )
                    )

                # Rebuild the Net:
                if len(collapse_dict) > 0:
                    all_collapse_dict[model_id] = {
                        "collapse_dict": collapse_dict,
                        "new_layer_info": new_layer_info,
                        "collapse_layer_ids": [
                            idx for item in collapse_dict.values() for idx in item
                        ],
                    }
            ...
            if len(all_collapse_dict) > 0:
                ...
                for model_id, model_ele in enumerate(model):
                    if model_id in all_collapse_dict:
                        W_list, b_list = model_ele.get_weights_bias(
                            W_source="core", b_source="core"
                        )
                        W_init_list = []
                        b_init_list = []
                        struct_param = []
                        for k in range(len(model_ele.struct_param)):
                            if (
                                k
                                not in all_collapse_dict[model_id]["collapse_layer_ids"]
                            ):
                                ...
                                if (
                                    k
                                    in all_collapse_dict[model_id][
                                        "collapse_dict"
                                    ].keys()
                                ):
                                    struct_param.append(
                                        all_collapse_dict[model_id]["new_layer_info"][
                                            k
                                        ]["layer_struct_param"]
                                    )
                                    W_init_list.append(
                        ...
                        model_ele_new = MLP(
                            input_size=model_ele.input_size,
                ...
                if verbose >= 1:
                    print(
                        "after collapsing linear layers in all models, new loss {}".format(
                            loss_new
                        )
                    )
                ...
                if "param" in record_keys:
                    param_record_whole.append(
                        model[0].get_weights_bias(W_source="core", b_source="core")
                    )
                if "param_grad" in record_keys:
                    param_grad_record_whole.append(
                        model[0].get_weights_bias(
                            W_source="core", b_source="core", is_grad=True
                        )
                    )
            ...
            if mode_ele == "snap":
                target_params = [
                    [(model_id, layer_id), "snap"]
                    for model_id, model_ele in enumerate(model)
                    for layer_id in range(len(model_ele.struct_param))
                ]
            elif mode_ele == "local":
                for model_id, model_ele in enumerate(model):
                ...
                target_params = [
                    [
                        ...
                        [
                            [
                                (("weight", (i, j)), 0.0)
                                for j in range(first_layer.output_size)
                            ]
                            for i in range(first_layer.input_size)
                        ],
                    ]
                ]
            else:
            ...
            for (model_id, layer_id), target_list in target_params:
                ...
                if isinstance(target_list, list):
                    ...
                    raise Exception(
                        "target_list {} not recognizable!".format(target_list)
                    )
                if verbose >= 2:
                    print("\n****starting model:****")
                    model[model_id].get_weights_bias(
                        W_source="core", b_source="core", verbose=True
                    )
                    print("********\n")

                performance_monitor.reset()
                criteria_value, criteria_result = get_criteria_value(
                    model,
                    X,
                    y,
                    criteria_type=simplify_criteria[0],
                    criterion=criterion,
                    **kwargs
                )
                to_stop, pivot_dict, log, _, pivot_id = performance_monitor.monitor(
                    criteria_value,
                    model_dict=model[model_id].model_dict,
                ...
                for i in range(max_passes):
                    ...
                    if isinstance(target_list, list):
                        info = layer.simplify(
                            mode="snap",
                            excluded_idx=excluded_idx_dict[(model_id, layer_id)],
                            snap_targets=target_list[i],
                            **kwargs
                        )
                    else:
                        info = layer.simplify(
                    ...
                    if len(info) == 0:
                        ...
                        print(
                            "Pass {0}, (model {1}, layer {2}) has no parameters to snap. Revert to pivot model. Go to next layer".format(
                                i, model_id, layer_id
                            )
                        )
                        break
                    excluded_idx_dict[(model_id, layer_id)] = (
                        excluded_idx_dict[(model_id, layer_id)] + info
                    )

                    _, loss_new, data_record = train_simple(
                        model,
                        X,
                        y,
                        optim_type="adam",
                        ...
                        if verbose >= 1:
                            print(
                                "Pass {0}, loss: {1}\tDL: {2}. New snap {3} is do not improve by {4} = {5} for {6} steps. Revert the simplification to pivot model. Go to next layer.".format(
                                    i,
                                    view_item(log, ("criteria_result", "loss")),
                                    view_item(log, ("criteria_result", "DL")),
                                    info,
                                    simplify_criteria[0],
                                    simplify_epsilon,
                                    simplify_patience,
                    ...
                    if layer_struct_param[1] == "Symbolic_Layer":
                        ...
                        for i in range(max_passes):
                            ...
                            if len(info) == 0:
                                target_layer_ids_exclude.append((model_id, layer_id))
                                print(
                                    "Pass {0}, (model {1}, layer {2}) has no parameters to pair_snap. Revert to pivot model. Go to next layer".format(
                                        i, model_id, layer_id
                                    )
                                )
                                break
                            _, loss, data_record = train_simple(
                                model,
                                X,
                                y,
                                optim_type="adam",
                            ...
                            if verbose >= 1:
                                ...
                                print(
                                    "symbolic_expression: ", layer.symbolic_expression
                                )
                                print(
                                    "numerical_expression: ", layer.numerical_expression
                                )
                                print()
                        ...
                        print(
                            "final: \nsymbolic_expression: ",
                            getattr(
                                model[model_id], "layer_{0}".format(layer_id)
                            ).symbolic_expression,
                        )
                        print(
            ...
            force_simplification = (
                kwargs["force_simplification"]
                if "force_simplification" in kwargs
                else False
            )
            ...
            for model_id, model_ele in enumerate(model):
                for layer_id, layer_struct_param in enumerate(model_ele.struct_param):
                    ...
                    if layer_struct_param[1] == "Simple_Layer":
                        ...
                        layer = getattr(model_ele, "layer_{}".format(layer_id))
                        if X is not None:
                            criteria_prev, criteria_result_prev = get_criteria_value(
                                model,
                                X,
                                y,
                                criteria_type=simplify_criteria[0],
                        ...
                        if mode_ele.split("_")[-1] == "separable":
                            ...
                            new_layer = Simple_2_Symbolic(
                                layer, settings=model_ele.settings, prefix=prefix
                            )
                        model[model_id].reset_layer(layer_id, new_layer)

                        if (
                            "snap_dict" in model_ele.settings
                            and layer_id in model_ele.settings["snap_dict"]
                        ):
                            subs_targets = []
                            for (pos, true_idx), item in model_ele.settings[
                                "snap_dict"
                            ][layer_id].items():
                                if pos == "weight":
                                    subs_targets.append(
                                    ...
                                    subs_targets.append(
                                        (
                                            Symbol("b{}".format(true_idx)),
                                            item["new_value"],
                                        )
                                    )
                                else:
                                    raise Exception(
                                        "pos {} not recognized!".format(pos)
                                    )
                            new_expression = [
                                expression.subs(subs_targets)
                                for expression in new_layer.symbolic_expression
                            ]
                            new_layer.set_symbolic_expression(new_expression)
                            model_ele.settings["snap_dict"].pop(layer_id)
                            model_ele.struct_param[layer_id][2].update(
                                new_layer.struct_param[2]
                            )

                        # Calculate the loss again:
                        if X is not None:
                            criteria_new, criteria_result_new = get_criteria_value(
                                model,
                                X,
                                y,
                                criteria_type=simplify_criteria[0],
                            ...
                            if verbose >= 1:
                                print(
                                    "Prev_loss: {0}, new loss: {1}\tprev_DL: {2:.9f}, new DL: {3:.9f}".format(
                                        criteria_result_prev["loss"],
                                        criteria_result_new["loss"],
                                        criteria_result_prev["DL"],
                                        criteria_result_new["DL"],
                                    )
                                )
                                print()
                            ...
                            print(
                                "{0} succeed. Prev_loss: {1}\tnew_loss: {2}\tprev_DL: {3:.9f}, new_DL: {4:.9f}".format(
                                    mode_ele,
                                    criteria_result_prev["loss"],
                                    criteria_result_new["loss"],
                                    criteria_result_prev["DL"],
            ...
            numerical_threshold = (
                kwargs["numerical_threshold"]
                if "numerical_threshold" in kwargs
                else None
            )
            is_numerical = kwargs["is_numerical"] if "is_numerical" in kwargs else False
            max_poly_degree = (
                kwargs["max_poly_degree"] if "max_poly_degree" in kwargs else None
            )
            show_before_truncate = (
                kwargs["show_before_truncate"]
                if "show_before_truncate" in kwargs
                else False
            )
            for model_id, model_ele in enumerate(model):
                ...
                if is_all_symbolic:
                    ...
                    for layer_id, layer_struct_param in enumerate(
                        model_ele.struct_param
                    ):
                        layer = getattr(model_ele, "layer_{0}".format(layer_id))
                        ...
                        for expr in layer_expression:
                            new_expr = expr.subs(
                                {
                                    "x{0}".format(i): "t{0}".format(i)
                                    for i in range(len(expression))
                                }
                            )  # Use a temporary variable to prevent overriding
                            new_expr = new_expr.subs(
                                {
                                    "t{0}".format(i): expression[i]
                                    for i in range(len(expression))
                                }
                            )
                    ...
                    model_ele_candidate = MLP(
                        input_size=model[0].layer_0.input_size,
                        struct_param=[
                            [
                                layer.output_size,
                                "Symbolic_Layer",
                                {"symbolic_expression": "x0"},
                            ]
                        ],
                        settings={},
                        is_cuda=model_ele.is_cuda,
                        ...
                        for expr in expression:
                            ...
                            for degree, coeff in zip(degree_list, coeff_list):
                                new_expr += (
                                    prod(
                                        [
                                            variables["x{0}".format(i)] ** degree[i]
                    ...
                    if not is_numerical:
                        ...
                        expression_new_all = []
                        for expr in expression:
                            expression_new, param_dict = numerical_2_parameter(
                                expr,
                                idx=len(param_dict_all),
                                threshold=numerical_threshold,
                            )
                            expression_new_all.append(expression_new)
                            param_dict_all.update(param_dict)
                        model_ele_candidate.layer_0.set_symbolic_expression(
                            expression_new_all, p_init=param_dict_all
                        )
                    else:
            ...
            def get_sign_snap_candidate(
                layer, activation_source, excluded_neurons=None
            ):
                coeff_dict = {}
                for i in range(len(layer.symbolic_expression)):
                    ...
                    if activation_source in func_names:
                        coeff = [
                            element
                            for element in layer.get_param_name_list(current_expression)
                            if element[0] == "W"
                        ]
                        coeff_dict[i] = np.mean(
                            [
                                np.abs(value)
                                for key, value in layer.get_param_dict().items()
                                if key in coeff
                            ]
                        )
                best_idx = None
            ...
            activation_source = (
                kwargs["activation_source"]
                if "activation_source" in kwargs
                else "sigmoid"
            )
            activation_target = (
                kwargs["activation_target"]
                if "activation_target" in kwargs
                else "heaviside"
            )
                ...
                for layer_id, layer_struct_param in enumerate(model_ele.struct_param):
                    if layer_struct_param[1] == "Symbolic_Layer":
                        ...
                        for i in range(layer_struct_param[0]):
                            # Obtain loss before simplification:
                            layer = getattr(model_ele, "layer_{0}".format(layer_id))
                            best_idx, _ = get_sign_snap_candidate(
                                layer,
                                activation_source,
                            ...
                            new_expression = [
                                expression.subs(
                                    activation_fun_source, activation_fun_target
                                )
                                if j == best_idx
                                else expression
                                for j, expression in enumerate(
                                    layer.symbolic_expression
                                )
                            ]
                            print(
                                "Pass {0}, candidate new expression: {1}".format(
                                    i, new_expression
                                )
                            )
                            ...
                            # Train:
                            _, loss_new, data_record = train_simple(
                                model, X, y, validation_data=validation_data, **kwargs
                            )
                            ...
                            # Check if the criterion after simplification and refit is worse. If it is worse than the simplify_epsilon, revert:
                            if to_stop:
                                model[model_id].load_model_dict(
                                    pivot_dict["model_dict"]
                                )
                                if verbose >= 1:
                                    print(
                                        "Pass {0}, loss: {1}\tDL: {2}. New snap {3} is do not improve by {4} = {5} for {6} steps. Revert the simplification to pivot model. Continue".format(
                                            i,
                                            view_item(log, ("criteria_result", "loss")),
                                            view_item(log, ("criteria_result", "DL")),
                                            info,
                            ...
                            if verbose >= 1:
                                print(
                                    "{0} succeed at (model {1}, layer {2}). loss: {3}\tDL: {4}".format(
                                        mode_ele,
                                        model_id,
                                        layer_id,
                                        view_item(log, ("criteria_result", "loss")),
            ...
            loss_list_specific = []
            ramping_L1_list = (
                kwargs["ramping_L1_list"]
                if "ramping_L1_list" in kwargs
                else np.logspace(-7, -1, 30)
            )
            ramping_mse_threshold = (
                kwargs["ramping_mse_threshold"]
                if "ramping_mse_threshold" in kwargs
                else 1e-5
            )
            ramping_final_multiplier = (
                kwargs["ramping_final_multiplier"]
                if "ramping_final_multiplier" in kwargs
                else 1e-2
            )
            layer_dict_dict = {}
            for i, L1_amp in enumerate(ramping_L1_list):
                reg_dict = {"weight": L1_amp, "bias": L1_amp, "param": L1_amp}
                _, loss_end, data_record = train_simple(
                    model,
                    X,
                    y,
                    reg_dict=reg_dict,
                ...
                print(
                    "L1-amp: {0}\tloss: {1}\tweight: {2}\tbias: {3}".format(
                        L1_amp, loss_end, weight, bias
                    )
                )
                ...
                if loss_end > ramping_mse_threshold:
                    if len(loss_list_specific) == 1:
                        print(
                            "\nThe MSE after the first L1-amp={0} is already larger than the ramping_mse_threshold. Stop and use current L1-amp. The figures will look empty.".format(
                                ramping_mse_threshold
                            )
                        )
                    else:
                        print(
                            "\nThe MSE {0} is larger than the ramping_mse_threshold {1}, stop ramping-L1 simplification".format(
                                loss_end, ramping_mse_threshold
                            )
                        )
                    break
            ...
            if "param" in record_keys:
                print(
                    "Final param value:\nweights: {0}\nbias{1}".format(
                        param_record_whole[final_L1_idx][0],
                        param_record_whole[final_L1_idx][1],
                    )
                )
            ...
            if isplot:
                ...
                plt.loglog(
                    ramping_L1_list[: len(loss_list_specific)], loss_list_specific
                )
                ...
                for i in range(weights_list.shape[1]):
                    plt.semilogx(
                        ramping_L1_list[: len(loss_list_specific)],
                        weights_list[:, i],
                        label="weight_{0}".format(i),
                    )
                if len(bias_list) > 0:
                    plt.semilogx(
                        ramping_L1_list[: len(loss_list_specific)],
...
class MLP(nn.Module):
    ...
    def struct_param(self):
        return [
            getattr(self, "layer_{0}".format(i)).struct_param
    ...
    def structure(self):
        ...
        structure["struct_param"] = (
            self.struct_param if hasattr(self, "struct_param") else None
        )
        return structure

    def init_layers(self, struct_param):
        res_forward = (
            self.settings["res_forward"] if "res_forward" in self.settings else False
        )
        for k, layer_struct_param in enumerate(struct_param):
            if res_forward:
                num_neurons_prev = (
                    struct_param[k - 1][0] + self.input_size
                    if k > 0
    ...
    def forward(self, *input, p_dict=None, **kwargs):
        kwargs = filter_kwargs(
            kwargs, ["res_forward", "is_res_block", "act_noise_scale"]
        )  # only allow certain kwargs to be passed
        ...
        is_res_block = (
            self.settings["is_res_block"] if "is_res_block" in self.settings else False
        )
        for k in range(len(self.struct_param)):
            p_dict_ele = p_dict[k] if p_dict is not None else None
            if res_forward and k > 0:
                output = getattr(self, "layer_{}".format(k))(
                    torch.cat([output, input], -1), p_dict=p_dict_ele, **kwargs
                )
            else:
                output = getattr(self, "layer_{}".format(k))(
                    output, p_dict=p_dict_ele, **kwargs
                )
        if is_res_block:
            output = output + input
        return output
    ...
    def snap(self, snap_mode="integer", top=5, **kwargs):
        ...
        if not hasattr(self, "num_layers") or self.num_layers != 1:
            ...
            for top_ele in range(1, top + 1):
                ...
                info_list = layer.simplify(
                    mode="snap", top=top_ele, snap_mode=snap_mode
                )
                if len(info_list) > 0:
    ...
    def insert_layer(self, layer_id, layer):
        ...
        if layer_id < self.num_layers - 1:
            ...
            if next_layer.struct_param[1] == "Simple_Layer":
                assert (
                    next_layer.input_size == layer.output_size
                ), "The inserted layer's output_size {0} must be compatible with next layer_{1}'s input_size {2}!".format(
                    layer.output_size, layer_id + 1, next_layer.input_size
                )
    ...
    def remove_layer(self, layer_id):
        ...
        if layer_id < self.num_layers - 1:
            num_neurons_prev = (
                self.struct_param[layer_id - 1][0] if layer_id > 0 else self.input_size
            )
            replaced_layer = getattr(self, "layer_{}".format(layer_id + 1))
            if replaced_layer.struct_param[1] == "Simple_Layer":
                assert (
                    replaced_layer.input_size == num_neurons_prev
                ), "After deleting layer_{0}, the replaced layer's input_size {1} must be compatible with previous layer's output neurons {2}!".format(
                    layer_id, replaced_layer.input_size, num_neurons_prev
                )
    ...
    def inspect_operation(self, input, operation_between, p_dict=None, **kwargs):
        ...
        for k in range(*operation_between):
            ...
            if res_forward and k > 0:
                output = getattr(self, "layer_{}".format(k))(
                    torch.cat([output, input], -1), p_dict=p_dict_ele
                )
            else:
        ...
        if W_source is not None:
            for k in range(len(self.struct_param)):
                if k in layer_ids:
                    if W_source == "core":
                        try:
                            W, _ = getattr(self, "layer_{}".format(k)).get_weights_bias(
                                is_grad=is_grad
                            )
                        ...
                        raise Exception(
                            "W_source '{}' not recognized!".format(W_source)
                        )
                    W_list.append(W)
        ...
        if b_source is not None:
            for k in range(len(self.struct_param)):
                if k in layer_ids:
                    if b_source == "core":
                        try:
                            _, b = getattr(self, "layer_{}".format(k)).get_weights_bias(
                                is_grad=is_grad
                            )
                        except Exception as e:
                            if raise_error:
                        ...
                        raise Exception(
                            "b_source '{}' not recognized!".format(b_source)
                        )
                b_list.append(b)
    ...
    def split_to_model_ensemble(self, mode="standardize"):
        ...
        if mode == "standardize":
            last_layer = getattr(
                model_core, "layer_{}".format(model_core.num_layers - 1)
            )
    ...
    def model_dict(self):
        ...
        model_dict["struct_param"] = get_full_struct_param(
            self.struct_param, self.settings
        )
        model_dict["weights"], model_dict["bias"] = self.get_weights_bias(
            W_source="core", b_source="core"
        )
    ...
    def DL(self):
        return np.sum(
            [getattr(self, "layer_{}".format(i)).DL for i in range(self.num_layers)]
        )
    ...
    def get_sympy_expression(self, verbose=True):
        ...
        for i in range(self.num_layers):
            ...
            if layer.struct_param[1] == "Symbolic_Layer":
                if verbose:
                    print(
                        "Layer {}, symbolic_expression:  {}".format(
                            i, layer.symbolic_expression
                        )
                    )
                    print(
                        "          numerical_expression: {}".format(
                            layer.numerical_expression
                        )
                    )
...
class Labelmix_MLP(nn.Module):
    def __init__(
        self, input_size, struct_param, idx_label=None, is_cuda=False,
    ):
        ...
        if idx_label is not None:
            ...
            idx_main = list(
                set(range(input_size))
                - set(to_np_array(idx_label).astype(int).tolist())
            )
        ...
        for i, layer_struct_param in enumerate(struct_param):
            ...
            setattr(
                self,
                "W_{}_main".format(i),
                nn.Parameter(torch.randn(num_neurons_prev, num_neurons)),
            )
            ...
            if self.idx_label is not None:
                setattr(
                    self,
                    "W_{}_mul".format(i),
                    nn.Parameter(torch.randn(len(self.idx_label), num_neurons)),
                )
                setattr(
                    self,
                    "W_{}_add".format(i),
                ...
                setattr(
                    self, "b_{}_mul".format(i), nn.Parameter(torch.zeros(num_neurons))
                )
                setattr(
                    self, "b_{}_add".format(i), nn.Parameter(torch.zeros(num_neurons))
                )
    ...
    def forward(self, input):
        ...
        for i, layer_struct_param in enumerate(self.struct_param):
            output = torch.matmul(
                output, getattr(self, "W_{}_main".format(i))
            ) + getattr(self, "b_{}_main".format(i))
            if "activation" in layer_struct_param[2]:
                output = get_activation(layer_struct_param[2]["activation"])(output)
            if self.idx_label is not None:
                A_mul = torch.matmul(
                    labels, getattr(self, "W_{}_mul".format(i))
                ) + getattr(self, "b_{}_mul".format(i))
                A_add = torch.matmul(
                    labels, getattr(self, "W_{}_add".format(i))
                ) + getattr(self, "b_{}_add".format(i))
...
class Multi_MLP(nn.Module):
    ...
    def get_regularization(self, source=["weight", "bias"], mode="L1", **kwargs):
        ...
        if self.is_cuda:
            reg = reg.cuda()
        for i in range(self.num_blocks):
            reg = reg + getattr(self, "block_{0}".format(i)).get_regularization(
    ...
    def struct_param(self):
        return [
            getattr(self, "block_{0}".format(i)).struct_param
            for i in range(self.num_blocks)
        ]
    ...
    def get_weights_bias(self, W_source="core", b_source="core"):
        ...
        b_list = []
        for i in range(self.num_blocks):
            W, b = getattr(self, "block_{0}".format(i)).get_weights_bias(
                W_source=W_source, b_source=b_source
            )
            W_list.append(W)
            b_list.append(b)
...
class Branching_Net(nn.Module):
    ...
    def __init__(
        self, net_base_model_dict, net_1_model_dict, net_2_model_dict, is_cuda=False,
    ):
    ...
    def get_regularization(self, source=["weights", "bias"], mode="L1"):
        reg = (
            self.net_base.get_regularization(source=source, mode=mode)
...
class Fan_in_MLP(nn.Module):
    def __init__(
        self, model_dict_branch1, model_dict_branch2, model_dict_joint, is_cuda=False,
    ):
    ...
    def model_dict(self):
        model_dict = {"type": self.__class__.__name__}
        model_dict["model_dict_branch1"] = (
            self.net_branch1.model_dict if self.net_branch1 is not None else None
        )
        model_dict["model_dict_branch2"] = (
            self.net_branch2.model_dict if self.net_branch2 is not None else None
        )
...
class Mixture_Model(nn.Module):
    def __init__(
        self, model_dict_list, weight_logits_model_dict, num_components, is_cuda=False,
    ):
        ...
        for i in range(self.num_components):
            if isinstance(model_dict_list, list):
                setattr(
                    self,
                    "model_{}".format(i),
                    load_model_dict(model_dict_list[i], is_cuda=is_cuda),
                )
            else:
                ...
                setattr(
                    self,
                    "model_{}".format(i),
                    load_model_dict(model_dict_list, is_cuda=is_cuda),
                )
        self.weight_logits_model = load_model_dict(
            weight_logits_model_dict, is_cuda=is_cuda
        )
        self.is_cuda = is_cuda
    ...
    def model_dict(self):
        model_dict = {
            "type": "Mixture_Model",
            "model_dict_list": [
                getattr(self, "model_{}".format(i)).model_dict
                for i in range(self.num_components)
            ],
...
class Model_Ensemble(nn.Module):
        ...
        for i in range(self.num_models):
            ...
            if net_type == "MLP":
                net = MLP(
                    ...
                    W_init_list=deepcopy(W_init_list[i])
                    if W_init_list is not None
                    else None,
                    b_init_list=deepcopy(b_init_list[i])
                    if b_init_list is not None
                    else None,
    ...
    def struct_param(self):
        return tuple(
            getattr(self, "model_{0}".format(i)).struct_param
            for i in range(self.num_models)
        )
    ...
    def settings(self):
        return [
            getattr(self, "model_{0}".format(i)).settings
            for i in range(self.num_models)
        ]
    ...
    def init_bias_with_input(self, input, mode="std_sqrt", neglect_last_layer=True):
        for i in range(self.num_models):
            ...
            model.init_bias_with_input(
                input, mode=mode, neglect_last_layer=neglect_last_layer
            )
    ...
    def get_regularization(self, source=["weight", "bias"], mode="L1", **kwargs):
        ...
        if "elastic_weight" in source or "elastic_bias" in source:
            ...
            excluded_layer = (
                kwargs["excluded_layer"] if "excluded_layer" in kwargs else [-1]
            )
            if not isinstance(excluded_layer, list):
                excluded_layer = [excluded_layer]
            excluded_layer = [
                element + model0.num_layers if element < 0 else element
                for element in excluded_layer
            ]
            ...
            for k in range(model0.num_layers):
                ...
                b_accum_k = []
                num_neurons_prev = (
                    model0.struct_param[k - 1][0] if k > 0 else self.input_size
                )
                ...
                for i in range(self.num_models):
                    ...
                    assert (
                        num_neurons_prev == model.struct_param[k - 1][0]
                        if k > 0
                        else model.input_size
                    ), "all models' input/output size at each layer must be identical!"
                    assert (
                        num_neurons == model.struct_param[k][0]
                    ), "all models' input/output size at each layer must be identical!"
        ...
        for k in range(self.num_models):
            reg = reg + getattr(self, "model_{0}".format(k)).get_regularization(
                source=source_core, mode=mode, **kwargs
            )
        return reg

    def get_weights_bias(
        self, W_source=None, b_source=None, verbose=False, isplot=False
    ):
        W_list_dict = {}
        b_list_dict = {}
        for i in range(self.num_models):
            if verbose:
                print("\nmodel {0}:".format(i))
            W_list_dict[i], b_list_dict[i] = getattr(
                self, "model_{0}".format(i)
            ).get_weights_bias(
        ...
        if mode == "mean":
            ...
            for k in range(model0.num_layers):
                ...
                if current_mode == "mean":
                    ...
                    struct_param[-1][0] = sum(
                        [self.struct_param[i][-1][0] for i in range(self.num_models)]
                    )
                else:
            ...
            if i not in model_ids:
                if k != i:
                    setattr(
                        self,
                        "model_{0}".format(k),
                        getattr(self, "model_{0}".format(i)),
                    )
                k += 1
        num_models_new = k
    ...
    def simplify(
        self, X, y, idx, mode="full", validation_data=None, isplot=False, **kwargs
    ):
        def process_idx(idx):
            idx = idx.byte()
            if len(idx.size()) == 1:
        ...
        for i in range(self.num_models):
            ...
            if validation_data is not None:
                X_valid_chosen = torch.masked_select(
                    X_valid, idx_valid[:, i : i + 1]
                ).view(-1, X_valid.size(1))
                y_valid_chosen = torch.masked_select(
                    y_valid, idx_valid[:, i : i + 1]
                ).view(-1, y_valid.size(1))
            ...
            if len(X_chosen) == 0:
                print(
                    "The {0}'th model has no corresponding data to simplify with, skip.".format(
                        i
                    )
                )
            else:
                new_model, loss_dict["model_{0}".format(i)] = simplify(
                    model,
                    X_chosen,
                    y_chosen,
                    mode=mode,
                    validation_data=validation_data_chosen,
                    isplot=isplot,
                    target_name="model_{0}".format(i),
                    **kwargs
                )
    ...
    def get_sympy_expression(self):
        ...
        for k in range(self.num_models):
            print("\nmodel {0}:".format(k))
            expressions["model_{0}".format(k)] = getattr(
                self, "model_{0}".format(k)
    ...
    def DL(self):
        return np.sum(
            [getattr(self, "model_{0}".format(i)).DL for i in range(self.num_models)]
        )
    ...
    def model_dict(self):
        ...
        for i in range(self.num_models):
            model_dict["model_{0}".format(i)] = getattr(
                self, "model_{0}".format(i)
            ).model_dict
        model_dict["input_size"] = self.input_size
        model_dict["struct_param"] = self.struct_param
...
def load_model_dict_model_ensemble(model_dict, is_cuda=False):
    num_models = len(
        [model_name for model_name in model_dict if model_name[:6] == "model_"]
    )
    return Model_Ensemble(
        ...
        struct_param=tuple(
            [
                deepcopy(model_dict["model_{0}".format(i)]["struct_param"])
                for i in range(num_models)
            ]
        ),
        W_init_list=[
            deepcopy(model_dict["model_{0}".format(i)]["weights"])
            for i in range(num_models)
        ],
        b_init_list=[
            deepcopy(model_dict["model_{0}".format(i)]["bias"])
            for i in range(num_models)
        ],
        settings=[
            deepcopy(model_dict["model_{0}".format(i)]["settings"])
            for i in range(num_models)
        ],
...
class Model_with_uncertainty(nn.Module):
    ...
    def forward(self, input, noise_amp=None, **kwargs):
        return (
            self.model_pred(input, noise_amp=noise_amp, **kwargs),
            self.model_logstd(input, **kwargs),
        )
    ...
    def get_regularization(self, source=["weight", "bias"], mode="L1", **kwargs):
        return self.model_pred.get_regularization(
            source=source, mode=mode, **kwargs
        ) + self.model_logstd.get_regularization(source=source, mode=mode, **kwargs)
...
class LSTM(RNNCellBase):
    ...
    def __init__(
        ...
        self.output_net = MLP(
            ...
            struct_param=output_struct_param,
    ...
    def forward_one_step(self, input, hx):
        ...
        return self._backend.LSTMCell(
            input, hx, self.W_ih, self.W_hh, self.b_ih, self.b_hh,
        )
    ...
    def get_weights_bias(
        ...
        W_dict = OrderedDict()
        b_dict = OrderedDict()
        W_o, b_o = self.output_net.get_weights_bias(
            W_source=W_source, b_source=b_source
        )
        if W_source == "core":
...
class Wide_ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.mean(
            (-1, -2)
        )  # replacing the out= F.avg_pool2d(out, 8) which is sensitive to the input shape.
...
class ConvNet(nn.Module):
        ...
        if struct_param is not None:
            ...
            for i in range(len(self.struct_param)):
                if i > 0:
                    ...
                    k = 0
                if (
                    self.struct_param[i - k][1] == "Simple_Layer"
                    and isinstance(num_channels_prev, tuple)
                    and len(num_channels_prev) == 3
                ):
                ...
                if "layer_input_size" in layer_settings and isinstance(
                    layer_settings["layer_input_size"], tuple
                ):
                ...
                if layer_type == "Conv2d":
                    layer = nn.Conv2d(
                        ...
                        if "stride" in layer_settings
                        else 1,
                        padding=layer_settings["padding"]
                        if "padding" in layer_settings
                        else 0,
                        dilation=layer_settings["dilation"]
                        if "dilation" in layer_settings
                        else 1,
                    )
                    ...
                    layer = nn.ConvTranspose2d(
                        ...
                        output_padding=layer_settings["output_padding"]
                        if "output_padding" in layer_settings
                        else 0,
                        dilation=layer_settings["dilation"]
                        if "dilation" in layer_settings
                        else 1,
                    ...
                    layer = get_Layer(
                        ...
                        if self.W_init_list is not None
                        and self.W_init_list[i] is not None
                        else None,
                        b_init=b_init_list[i]
                        if self.b_init_list is not None
                        and self.b_init_list[i] is not None
                        else None,
                        settings=layer_settings,
                        is_cuda=is_cuda,
                    )
                    ...
                    layer = nn.MaxPool2d(
                        ...
                        return_indices=layer_settings["return_indices"]
                        if "return_indices" in layer_settings
                        else False,
                    )
                elif layer_type == "MaxUnpool2d":
                    layer = nn.MaxUnpool2d(
                        kernel_size=layer_settings["kernel_size"],
                        stride=layer_settings["stride"]
                        if "stride" in layer_settings
                        else None,
                    ...
                    layer = nn.Upsample(
                        ...
                        mode=layer_settings["mode"]
                        if "mode" in layer_settings
                        else "nearest",
                    )
                ...
                if (
                    self.W_init_list is not None
                    and self.W_init_list[i] is not None
                    and layer_type not in ["Simple_Layer"]
                ):
    ...
    def forward(self, input, indices_list=None, **kwargs):
        return self.inspect_operation(
            input, operation_between=(0, self.num_layers), indices_list=indices_list
        )
    ...
    def inspect_operation(self, input, operation_between, indices_list=None):
        ...
        for i in range(start_layer, end_layer):
            if "layer_input_size" in self.struct_param[i][2]:
                ...
                assert (
                    output.shape[0] == output_size_last
                ), "output_size reshaped to different length. Check shape!"
            if "Unpool" in self.struct_param[i][1]:
                output_tentative = getattr(self, "layer_{0}".format(i))(
                    output, indices_list.pop(-1)
                )
            else:
            ...
            if "activation" in self.struct_param[i][2]:
                ...
                if "activation" in self.settings:
                    ...
                    activation = "linear"
                if (
                    "Pool" in self.struct_param[i][1]
                    or "Unpool" in self.struct_param[i][1]
                    or "Upsample" in self.struct_param[i][1]
                ):
                    activation = "linear"
...
class Conv_Model(nn.Module):
    ...
    def get_regularization(self, source=["weights", "bias"], mode="L1"):
        if self.is_generative:
            return self.core.get_regularization(
                source=source, mode=mode
            ) + self.decoder.get_regularization(source=source, mode=mode)
        else:
            return (
                self.encoder.get_regularization(source=source, mode=mode)
...
class Conv_Autoencoder(nn.Module):
    def __init__(
        ...
        self.encoder = ConvNet(
            ...
            struct_param=struct_param_encoder,
            settings=settings,
            is_cuda=is_cuda,
        )
        self.decoder = ConvNet(
            input_channels=input_channels_decoder,
            struct_param=struct_param_decoder,
            settings=settings,
            is_cuda=is_cuda,
        )
        self.is_cuda = is_cuda
    ...
    def get_regularization(self, source=["weight", "bias"], mode="L1"):
        return self.encoder.get_regularization(
            source=source, mode=mode
        ) + self.decoder.get_regularization(source=source, mode=mode)
...
class VAE(nn.Module):
    def __init__(
        self, encoder_model_dict, decoder_model_dict, is_cuda=False,
    ):
...
class Net_reparam(nn.Module):
    ...
    def __init__(
        self, model_dict, reparam_mode, is_cuda=False,
    ):
...
def reparameterize_mixture_diagonal(model, input, mode):
    ...
    if mode.startswith("diagg"):
        ...
        scale_list = (
            F.softplus(mean_logit[:, size:], beta=1) + 0.01
        )  # Avoid the std to go to 0
    dist = Mixture_Gaussian_reparam(
        mean_list=mean_list, scale_list=scale_list, weight_logits=weight_logits,
    )
...
class Mixture_Gaussian(nn.Module):
    def __init__(
        self, num_components, dim, param_mode="full", is_cuda=False,
    ):
    ...
    def initialize(self, model_dict=None, input=None, num_samples=100, verbose=False):
        if input is not None:
            ...
            for i in range(num_samples):
                neg_log_prob, loc_init_list, scale_init_list = self.initialize_ele(
                    input
                )
                if verbose:
            ...
            if model_dict is None:
                self.weight_logits = nn.Parameter(
                    (torch.randn(self.num_components) * np.sqrt(2 / (1 + self.dim))).to(
                        self.device
                    )
                )
            else:
                self.weight_logits = nn.Parameter(
                    (torch.FloatTensor(model_dict["weight_logits"])).to(self.device)
                )
            ...
            if model_dict is None:
                self.loc_list = nn.Parameter(
                    torch.randn(self.num_components, self.dim).to(self.device)
                )
                self.scale_list = nn.Parameter(
                    (torch.randn(self.num_components, size) / self.dim).to(self.device)
                )
            else:
                self.loc_list = nn.Parameter(
                    torch.FloatTensor(model_dict["loc_list"]).to(self.device)
                )
                self.scale_list = nn.Parameter(
                    torch.FloatTensor(model_dict["scale_list"]).to(self.device)
                )
    ...
    def initialize_ele(self, input):
        ...
        length = len(input)
        self.weight_logits = nn.Parameter(
            torch.zeros(self.num_components).to(self.device)
        )
        self.loc_list = nn.Parameter(
            input[
                torch.multinomial(torch.ones(length) / length, self.num_components)
            ].detach()
        )
        self.scale_list = nn.Parameter(
            (
                torch.randn(self.num_components, size).to(self.device) * input.std() / 5
            ).to(self.device)
        )
    ...
    def prob(self, input):
        ...
        if self.param_mode == "diag":
            ...
            logits = (
                -((input - self.loc_list) ** 2) / 2 / scale_list ** 2
                - torch.log(scale_list * np.sqrt(2 * np.pi))
            ).sum(-1)
        else:
            raise
    ...
    def visualize(self, input):
        ...
        X = np.arange(
            to_np_array(input.min()) - 0.2 * std,
            to_np_array(input.max()) + 0.2 * std,
            0.1,
        )
        Y_dict = {}
        ...
        for i in range(self.num_components):
            Y_dict[i] = weights[0].item() * scipy.stats.norm.pdf(
                (X - self.loc_list[i].item()) / self.scale_list[i].item()
            )
            plt.plot(X, Y_dict[i])
...
class Triangular_dist(Distribution):
    ...
    def icdf(self, value):
        ...
        out[iidx] = (
            loc[iidx]
            + b[iidx]
            - torch.sqrt(b[iidx] * a_plus_b[iidx] * (1 - value[iidx]))
        )
        return out
...
def load_model_dict_distribution(model_dict, is_cuda=False):
    if model_dict["type"] == "Mixture_Gaussian":
        ...
        model = Triangular_dist(
            loc=model_dict["loc"], a=model_dict["a"], b=model_dict["b"],
        )
    else:
