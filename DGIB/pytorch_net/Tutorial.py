...
struct_param = [
    [
        ...
        "Simple_Layer",
        {},
    ],  # (number of neurons in each layer, layer_type, layer settings)
    [400, "Simple_Layer", {"activation": "relu"}],
    [1, "Simple_Layer", {"activation": "linear"}],
]
settings = {
    "activation": "relu"
}  # Default activation if the activation is not specified in "struct_param" in each layer.
# If the activation is specified, it will overwrite this default settings.

net = MLP(input_size=input_size, struct_param=struct_param, settings=settings,)
...
# Save model:
pickle.dump(
    net.model_dict, open("net.p", "wb")
)  # net.model_dict constains all the information (structural and parameters) of the network
...
model_dict = {
    ...
    "struct_param": [
        [
            ...
            "Symbolic_Layer",
            {
                "symbolic_expression": "[3 * x0 ** 2 + p0 * x1 * x2 + p1 * x3, 5 * x0 ** 2 + p2 * x1 + p3 * x3 * x2]"
            },
        ],
    ...
    "weights": [{"p0": -1.3, "p1": 1.0, "p2": 2.3, "p3": -0.4}, {"p2": -1.5},],
}
...
net = MLP(input_size=input_size, struct_param=struct_param, settings={},)
...
layer_dict = {
    ...
    "settings": {
        "W_available": [
            "dense",
            "Toeplitz",
        ],  # Weight type. Choose subset of "dense", "Toeplitz", "arithmetic-series-in", "arithmetic-series-out", "arithmetic-series-2D-in", "arithmetic-series-2D-out"
        "b_available": [
            "dense",
            "None",
            "arithmetic-series",
            "constant",
        ],  # Bias type. Choose subsets of "None", "constant", "arithmetic-series", "arithmetic-series-2D"
        "A_available": [
            "linear",
            "relu",
        ],  # Activation. Choose subset of "linear", "relu", "leakyRelu", "softplus", "sigmoid", "tanh", "selu", "elu", "softmax"
    },
}
...
struct_param = [
    ...
    [
        ...
        "SuperNet_Layer",
        {
            "activation": "relu",  # Choose from "linear", "relu", "leakyRelu", "softplus", "sigmoid", "tanh", "selu", "elu", "softmax"
...
dataset_train = data_utils.TensorDataset(
    X_train.data, y_train.data
)  #  The data_loader must use the torch Tensor, not Variable. So I use X_train.data to get the Tensor.
...
for epoch in range(epochs):
    ...
    print(
        "epoch {0} \tbatch {1} \tloss_train: {2:.6f}\tloss_test: {3:.6f}".format(
            epoch, batch_id, loss_train.item(), loss_test.item()
        )
    )
    if to_stop:
...
model = ConvNet(input_channels=1, struct_param=struct_param_conv, is_cuda=is_cuda,)
...
dataset_name = "MNIST"
batch_size = 128
dataset_raw = getattr(datasets, dataset_name)(
    "datasets/{0}".format(dataset_name), download=True
)
X, y = to_Variable(
    dataset_raw.train_data.unsqueeze(1).float(),
    dataset_raw.train_labels,
    is_cuda=is_cuda,
)
...
loss_original, loss_value, data_record = train(
    model,
    ...
    if isplot:
        ...
        if dim == 3:
            ...
            ax.text2D(
                0.87,
                0.92,
                "Iteration: {:d}\nError: {:06.4f}".format(iteration, loss),
