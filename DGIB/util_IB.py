...
sys.path.append(os.path.join(os.path.dirname("__file__"), "..", ".."))
from DGIB.pytorch_net.util import to_np_array
...
def get_root_dir():
    ...
    index = dirname_split.index("DGIB")
    dirname = "/".join(dirname_split[: index + 1])
    return dirname


DGIB_PATH = get_root_dir() + "/results"
...
def sample_lognormal(mean, sigma=None, sigma0=1.0):
    """
...
def compose_log(metrics, key, spaces=0, tabs=0, newline=False):
    ...
    return (
        ...
        + "{}: ({:.4f}, {:.4f}, {:.4f})".format(
            key,
            metrics["train_{}".format(key)],
            metrics["val_{}".format(key)],
            metrics["test_{}".format(key)],
        )
    )
...
def edge_index_2_csr(edge_index, size):
    ...
    csr_matrix = sp.csr_matrix(
        (np.ones(edge_index.shape[1]), to_np_array(edge_index)), shape=(size, size)
    )
    return csr_matrix
...
def select_nodes(data, info, data_type, num=10, seed=None):
    ...
    try:
        ...
        print(
            "Save attacked node_ids to {}:test-node_n:{}.pkl.".format(attack_path, num)
        )
    return node_ids
...
def parse_filename(filename, is_adversarial=False, **kwargs):
    ...
    Args:
        ...
        is_adversarial: whether the file is from experiments/DGIB_node_adversarial_attack.
    """
    if is_adversarial:
...
def parse_filename_standard(filename):
    ...
    parse_dict["train_fraction"] = (
        eval(filename_split[filename_split.index("tr") + 1])
        if "tr" in filename_split
        else 1
    )
    parse_dict["added_edge_fraction"] = (
        eval(filename_split[filename_split.index("ed") + 1])
        if "ed" in filename_split
        else 0
    )
    parse_dict["feature_noise_ratio"] = (
        eval(filename_split[filename_split.index("ed") + 2])
        if filename_split[filename_split.index("ed") + 2] != "beta"
        else 0
    )
    ...
    parse_dict["num_layers"] = (
        int(filename_split[filename_split.index("lay") + 1])
        if "lay" in filename_split
        else 2
    )
    ...
    parse_dict["reparam_all_layers"] = eval(
        filename_split[filename_split.index("reall") + 1]
    )
    parse_dict["epochs"] = int(filename_split[filename_split.index("epochs") + 1])
    parse_dict["lr"] = (
        eval(filename_split[filename_split.index("lr") + 1])
        if "lr" in filename_split
        else -1
    )
    parse_dict["weight_decay"] = (
        eval(filename_split[filename_split.index("weight_decay") + 1])
        if "weight_decay" in filename_split
        else -1
    )
    parse_dict["seed"] = int(filename_split[filename_split.index("seed") + 1])
    parse_dict["idx"] = filename_split[filename_split.index("id") + 1][:-2]
    if baseline:
        parse_dict["threshold"] = float(
            filename_split[filename_split.index("threshold") + 1]
        )
        parse_dict["gamma"] = (
            float(filename_split[filename_split.index("gamma") + 1])
            if "gamma" in filename_split
            else None
        )
    else:
        ...
        parse_dict["struct_dropout_mode"] = (
            filename_split[filename_split.index("sdrop") + 1]
            if "sdrop" in filename_split
            else "(standard,0.6)"
        )
    return parse_dict
...
def parse_filename_adversarial(filename, baseline=False):
    ...
    parse_dict["n_perturbations"] = eval(
        filename_split[filename_split.index("pert") + 1]
    )
    parse_dict["latent_size"] = int(filename_split[filename_split.index("lat") + 1])
    parse_dict["sample_size"] = int(filename_split[filename_split.index("samp") + 1])
    parse_dict["num_layers"] = (
        eval(filename_split[filename_split.index("l") + 1])
        if "l" in filename_split
        else 2
    )
    parse_dict["is_anneal_beta"] = eval(filename_split[filename_split.index("anl") + 1])
    parse_dict["val_use_mean"] = eval(filename_split[filename_split.index("mean") + 1])
    ...
    parse_dict["weight_decay"] = (
        eval(filename_split[filename_split.index("l2") + 1])
        if "l2" in filename_split
        else -1
    )
    parse_dict["epochs"] = int(filename_split[filename_split.index("epochs") + 1])
    parse_dict["seed"] = int(filename_split[filename_split.index("seed") + 1])
    ...
    if not baseline:
        ...
        parse_dict["reparam_mode"] = filename_split[filename_split.index("reparam") + 1]
        parse_dict["prior_mode"] = filename_split[filename_split.index("prior") + 1]
        parse_dict["struct_dropout_mode"] = filename_split[
            filename_split.index("sdrop") + 1
        ]
    else:
        parse_dict["gamma"] = eval(filename_split[filename_split.index("gamma") + 1])
        parse_dict["threshold"] = eval(
            filename_split[filename_split.index("threshold") + 1][:-2]
        )
    return parse_dict
