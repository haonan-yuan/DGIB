...
class Runner(object):
    def __init__(self, args, model, data, writer=None, **kwargs):
        ...
        if len(data['x']) == self.len:
            x = [self.data['x'][t].to(args.device).clone().detach() for t in range(self.len)]
            self.x = x
        else:
    ...
    def train(self, epoch, data):
        ...
        embeddings, ixz_loss, structure_kl_loss, consensual_loss = self.model(self.x, [data['edge_index_list'][ix].long().to(args.device) for ix in range(self.len)])
        ...
        loss = main_loss + (1-alpha) * consensual_loss + beta_1 * ixz_loss + beta_2 * structure_kl_loss
    ...
    def run(self):
        ...
        self.optimizer = optim.Adam([p for n, p in self.model.named_parameters() if 'ss' not in n], lr=args.lr, weight_decay=args.weight_decay)
        ...
        with tqdm(range(1, args.max_epoch + 1)) as bar:
            for epoch in bar:
                ...
                if epoch == 1 or epoch % self.args.log_interval == 0:
                    ...
                    print(f"Current: Epoch:{epoch}, Train AUC:{average_train_auc:.4f}, Val AUC: {average_val_auc:.4f}, Test AUC: {average_test_auc:.4f}")
                    print(f"Train: Epoch:{test_results[0]}, Train AUC:{max_train_auc:.4f}, Val AUC: {max_auc:.4f}, Test AUC: {max_test_auc:.4f}")
                    print(f"Test: Epoch:{test_results[0]}, Train AUC:{test_results[1]:.4f}, Val AUC: {test_results[2]:.4f}, Test AUC: {test_results[3]:.4f}")
    ...
    def test(self, epoch, data):
        ...
        test_auc_list = []
        self.model.eval()
        embeddings, _, _, _ = self.model(self.x, [data['edge_index_list'][ix].long().to(args.device) for ix in range(self.len)])
    ...
    def re_run(self):
        ...
        self.model.eval()
        embeddings, _, _, _ = self.model(self.x, [data_clean['edge_index_list'][ix].long().to(args.device) for ix in range(self.len)])
        ...
        embeddings, _, _, _ = self.model(self.x, [data_attacked['edge_index_list'][ix].long().to(args.device) for ix in range(self.len)])
        ...
        embeddings, _, _, _ = self.model(self.x_noise, [data_attacked['edge_index_list'][ix].long().to(args.device) for ix in range(self.len)])
    ...
    def re_run_evasive(self):
        ...
        if self.args.distribution == 'Bernoulli':
            filepath = '../saved_model/original_evasive/bernoulli/' + self.args.dataset.split("_")[0] + '.pth'
        elif self.args.distribution == 'categorical':
            filepath = '../saved_model/original_evasive/categorical/' + self.args.dataset.split("_")[0] + '.pth'
        checkpoint = torch.load(filepath)
    ...
    def re_run_poisoning(self):
        ...
        if self.args.distribution == 'Bernoulli':
            filepath = '../saved_model/poisoning/bernoulli/' + self.args.dataset + '.pth'
        elif self.args.distribution == 'categorical':
            filepath = '../saved_model/poisoning/categorical/' + self.args.dataset + '.pth'
        checkpoint = torch.load(filepath)
