...
for i in tqdm(range(100)):
    ...
    adv_train_model.fit(
        features, modified_adj, labels, idx_train, train_iters=50, initialize=False
    )
...
print(
    "No adversarial training: classfication margin for {0} nodes: {1}".format(
        len(target_nodes), np.mean(all_margins)
    )
)

print(
    "Adversarial training: classfication margin for {0} nodes: {1}".format(
        len(target_nodes), np.mean(all_adv_margins)
    )
)
