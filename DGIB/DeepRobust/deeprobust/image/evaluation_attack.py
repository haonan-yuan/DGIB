    ...
    **kwargs
    ...
    for count, (data, target) in enumerate(test_loader):
        ...
        if random_targeted == True:
            ...
            adv_example = attackmethod.generate(
                data, target, target_label=target_label, **kwargs
            )
    ...
    print(
        "Attack Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n".format(
            ...
            100.0 * correct / (batch_num * batch_size),
        )
    )
...
def parameter_parser():
    parser = argparse.ArgumentParser(
        description="Run attack algorithms.", usage="Use -h for more information."
    )
    ...
    parser.add_argument("--device", default="cuda", help="Choose the device.")
    ...
    if args.attack_method == "PGD":
        ...
        try:
            if args.random_targeted == 0 and args.target_label == -1:
                raise ValueError(
                    "No target label assigned. Random generate target for each input."
                )
        except ValueError:
        ...
        run_attack(
            ...
            args.device,
            test_loader,
            random_targeted=args.random_targeted,
        ...
        test_loader = generate_dataloader(args.dataset, args.batch_size)
        run_attack(
            attack_method, args.batch_size, args.batch_num, args.device, test_loader
        )
