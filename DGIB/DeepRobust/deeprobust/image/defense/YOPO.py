...
class IPGDAttackMethodMaker(object):
    ...
    def __call__(self, DEVICE):
        return YOPOpgd.FASTPGD(
            self.eps, self.sigma, self.nb_iters, self.norm, DEVICE, self.mean, self.std
        )
...
class FastGradientLayerOneTrainer(object):
    def __init__(
        self, Hamiltonian_func, param_optimizer, inner_steps=2, sigma=0.008, eps=0.03
    ):
...
def main():
    ...
    create_optimizer = SGDOptimizerMaker(
        lr=1e-2 / K, momentum=0.9, weight_decay=weight_decay
    )

    create_lr_scheduler = PieceWiseConstantLrSchedulerMaker(
        milestones=[30, 35, 39], gamma=0.1
    )
    ...
    create_evaluation_attack_method = IPGDAttackMethodMaker(
        ...
        std=torch.tensor(
            np.array([1]).astype(np.float32)[np.newaxis, :, np.newaxis, np.newaxis]
        ),
    )
    ...
    parser.add_argument(
        "-b", "--batch_size", default=256, type=int, metavar="N", help="mini-batch size"
    )
    ...
    net = YOPOCNN.Net()
    net.to(DEVICE)
    criterion = CrossEntropyWithWeightPenlty(
        net.other_layers, DEVICE, weight_decay
    )  # .to(DEVICE)
    ...
    LayerOneTrainer = FastGradientLayerOneTrainer(
        Hamiltonian_func, layer_one_optimizer, inner_iters, sigma, eps
    )
    ...
    while True:
        ...
        descrip_str = "Training epoch:{}/{} -- lr:{}".format(
            now_epoch, num_epochs, lr_scheduler.get_lr()[0]
        )
        s_time = time.time()

        # train
        acc, yopoacc = train_one_epoch(
            net,
            ds_train,
            optimizer,
            eps,
        ...
        utils.save_checkpoint(
            ...
            file_name=os.path.join(
                args.model_dir, "epoch-{}.checkpoint".format(now_epoch)
            ),
        )
