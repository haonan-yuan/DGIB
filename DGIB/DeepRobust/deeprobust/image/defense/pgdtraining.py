...
class PGDtraining(BaseDefense):
            ...
            if self.save_model and epoch % 10 == 0:
                if os.path.isdir("./" + str(self.save_dir)):
                    torch.save(
                        ...
                        str(self.save_dir) + "/" + self.save_name,
                    )
                    print("model saved in " + "./" + str(self.save_dir))
                else:
                    print(
                        "make new directory and save model in "
                        + "./"
                        + str(self.save_dir)
                    )
    ...
    def test(self, model, device, test_loader):
        ...
        for data, target in test_loader:
            ...
            data_adv, output_adv = self.adv_data(
                data, target, ep=self.epsilon, num_steps=self.num_steps
            )

            test_loss_adv += self.calculate_loss(
                output_adv, target, redmode="sum"
            ).item()  # sum up batch loss
            pred_adv = output_adv.argmax(
                dim=1, keepdim=True
            )  # get the index of the max log-probability
