...
class TRADES(BaseDefense):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if epoch % self.save_freq == 0:
                torch.save(
                    ...
                    os.path.join(
                        self.save_dir, "trade_model-nn-epoch{}.pt".format(epoch)
                    ),
                )
                torch.save(
                    optimizer.state_dict(),
                    os.path.join(
                        self.save_dir, "opt-nn-checkpoint_epoch{}.tar".format(epoch)
                    ),
                )
    ...
    def test(self, model, device, test_loader):
        ...
        print(
            "Test: Clean loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)".format(
                test_loss,
                correct,
