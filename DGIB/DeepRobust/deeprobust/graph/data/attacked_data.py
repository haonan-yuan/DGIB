...
class PtbDataset:
    ...
    def __init__(self, root, name, attack_method="mettack", transform=None):
        assert (
            attack_method == "mettack" or attack_method == "metattack"
        ...
        self.data_filename = osp.join(
            root, "{}_{}_0.05.npz".format(self.name, self.attack_method)
        )
    ...
    def load_data(self):
        ...
        print(
            """UserWarning: the adjacency matrix is perturbed, using the data splits under seed 15(default seed for deeprobust.graph.data.Dataset), so if you are going to verify the attacking performance, you should use the same data splits"""
        )
        return adj
    ...
    def download_npz(self):
        ...
        try:
            ...
            raise Exception(
                """Download failed! Make sure you have
...
class PrePtbDataset:
    ...
    def __init__(self, root, name, attack_method="meta", ptb_rate=0.05, transform=None):
        ...
        self.url = "https://raw.githubusercontent.com/ChandlerBang/Pro-GNN/master/{}/{}_{}_adj_{}.npz".format(
            self.attack_method, self.name, self.attack_method, self.ptb_rate
        )
        self.root = osp.expanduser(osp.normpath(root))
        self.data_filename = osp.join(
            root,
    ...
    def load_data(self):
        if not osp.exists(self.data_filename):
            self.download_npz()
        print(
            "Loading {} dataset perturbed by {} {}...".format(
                self.name, self.ptb_rate, self.attack_method
            )
        )

        if self.attack_method == "meta":
            print(
                """UserWarning: the pre-attacked graph is perturbed, using the data splits under seed 15 (default seed), so if you are going to verify the attacking performance, you should use the same data splits."""
            )
            adj = sp.load_npz(self.data_filename)
...
if __name__ == "__main__":
    perturbed_data = PrePtbDataset(
        root="/tmp/", name="cora", attack_method="meta", ptb_rate=0.05
    )
