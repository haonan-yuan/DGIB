...
from DGIB.DeepRobust.deeprobust.graph.utils import (
    get_train_val_test,
...
class Dataset:
    def __init__(self, root, name, setting="nettack", seed=None, require_mask=False):
        ...
        self.url = (
            "https://raw.githubusercontent.com/danielzuegner/gnn-meta-attack/master/data/%s.npz"
            % self.name
        )
    ...
    def get_adj(self):
        ...
        assert (
            adj.max() == 1 and len(np.unique(adj[adj.nonzero()].A1)) == 1
