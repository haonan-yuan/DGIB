...
class NodeInjectionEnv(NodeAttackEnv):
    def __init__(
        ...
        super(NodeInjectionEnv, self).__init__(
            features, labels, idx_val, dict_of_lists, classifier
        )
    ...
    def step(self, actions, inference=False):
        ...
        if (self.n_steps + 1) % 3 == 0:
            for i in range(self.parallel_size):
                # change label
                self.modified_label_list[i][
                    self.first_nodes[i] - self.ori_adj_size
                ] = actions[i]
        ...
        if not inference:
            if self.isActionFinished():
                ...
                for i in range(self.parallel_size):
                    ...
                    loss, correct = loss_acc(
                        output, self.labels, self.idx_val, avg_loss=False
                    )
                    acc = correct.sum()
    ...
    def uniformRandActions(self):
        ...
        for i in range(self.parallel_size):
            ...
            if self.first_nodes is not None and self.second_nodes is None:
                ...
                while (self.first_nodes[i], cur_action) in self.modified_list[
                    i
                ].edge_set:
                    cur_action = np.random.randint(len(self.list_action_space))
    ...
    def cloneState(self):
        return list(
            zip(deepcopy(self.modified_list), deepcopy(self.modified_label_list))
        )
