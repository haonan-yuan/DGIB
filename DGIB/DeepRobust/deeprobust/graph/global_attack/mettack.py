...
class Metattack(BaseMeta):
    ...
    def get_meta_grad(
        self, features, adj_norm, idx_train, idx_unlabeled, labels, labels_self_training
    ):
        ...
        if self.lambda_ == 1:
            ...
            attack_loss = (
                self.lambda_ * loss_labeled + (1 - self.lambda_) * loss_unlabeled
            )

        print("GCN loss on unlabled data: {}".format(loss_test_val.item()))
        print(
            "GCN acc on unlabled data: {}".format(
        ...
        if self.attack_structure:
            adj_grad = torch.autograd.grad(
                attack_loss, self.adj_changes, retain_graph=True
            )[0]
        if self.attack_features:
            feature_grad = torch.autograd.grad(
                attack_loss, self.feature_changes, retain_graph=True
            )[0]
    ...
    def attack(
        ...
        for i in tqdm(range(perturbations), desc="Perturbing graph"):
            ...
            if self.attack_structure:
                adj_meta_score = self.get_adj_score(
                    adj_grad, modified_adj, ori_adj, ll_constraint, ll_cutoff
                )
            if self.attack_features:
            ...
            if adj_meta_score.max() >= feature_meta_score.max():
                ...
                row_idx, col_idx = utils.unravel_index(
                    feature_meta_argmax, ori_features.shape
                )
...
class MetaApprox(BaseMeta):
    def __init__(
        ...
        super(MetaApprox, self).__init__(
            model,
            nnodes,
            feature_shape,
            lambda_,
        ...
        self.optimizer = optim.Adam(
            self.weights + self.biases, lr=lr
        )  # , weight_decay=5e-4)
        self._initialize()
    ...
    def inner_train(
        ...
        for j in range(self.train_iters):
            ...
            if self.attack_structure:
                ...
                self.adj_grad_sum += torch.autograd.grad(
                    attack_loss, self.adj_changes, retain_graph=True
                )[0]
            if self.attack_features:
    ...
    def attack(
        ...
        for i in tqdm(range(perturbations), desc="Perturbing graph"):
            ...
            if self.attack_structure:
                adj_meta_score = self.get_adj_score(
                    self.adj_grad_sum, modified_adj, ori_adj, ll_constraint, ll_cutoff
                )
            if self.attack_features:
                feature_meta_score = self.get_feature_score(
