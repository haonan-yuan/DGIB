...
from DGIB.pytorch_net.util import sample, to_cpu_recur, to_np_array, to_Variable, record_data, make_dir, remove_duplicates, update_dict, get_list_elements, to_string, filter_filename
from DGIB.util_IB import get_reparam_num_neurons, sample_lognormal, scatter_sample, uniform_prior, compose_log, edge_index_2_csr, COLOR_LIST, LINESTYLE_LIST, process_data_for_nettack, parse_filename, add_distant_neighbors
...
class DGIBConv(MessagePassing):
    def __init__(self, in_channels, out_channels, heads=1, concat=True,
                 ...
                 distribution=None, temperature=0.2, nbsz=15,
                 val_use_mean=True, sample_size=1,
                 bias=True, agg_param = 0.8,
                 **kwargs):
    ...
    def forward(self, x_all, edge_index_all, size=None):
        ...
        for t in range(times):
            out, self.ixz_list[t], self.structure_kl_loss_list[t] = self.forward_single_graphshot(x_all[t], edge_index_all[t])
            if t > 0:
                weights = F.sigmoid(torch.tensor(list(range(t))).view(t, 1, 1).to(x_all[0].device))
                out_list[t] = (torch.sum(weights * out_list[:t], dim=0) / t) * self.agg_param + out * (1 - self.agg_param)
            else:
                out_list[t] = out
    ...
    def message(self, edge_index_i, x_i, x_j, size_i):
        ...
        if 'categorical' in neighbor_sampling_mode:
            ...
            self.alpha = alpha
            self.prior = (torch.ones_like(self.alpha)*(1/self.nbsz)).to(alpha.device)
...
class DGIBNN(torch.nn.Module):
    ...
    def time_encoding(self, x_all):
        ...
        if self.use_RTE:
            times = len(x_all)
            for t in range(times):
                x_all[t] = x_all[t] + self.lin(self.emb(torch.LongTensor([t]).to(x_all[t].device)))
        return x_all
    ...
    def init(self):
        ...
        for i in range(self.num_layers):
            ...
            setattr(self, "conv{}".format(i + 1), DGIBConv(
                input_size,
                latent_size,
                heads=self.args.heads if i != self.num_layers - 1 else 1, concat=True,
                reparam_mode=self.reparam_mode if is_reparam else None,
                prior_mode=self.prior_mode if is_reparam else None,
                distribution=self.distribution, temperature=self.temperature, nbsz=self.nbsz,
    ...
    def forward(self, x_all, edge_index_all):
        ...
        for i in range(self.num_layers):
            ...
            x_all, ixz_mean, structure_kl_loss_mean, ixz_cons = layer(x_all, edge_index_all)
            ixz.append(ixz_mean)
            structure_kl_loss.append(structure_kl_loss_mean)
            consensual.append(ixz_cons)

            x_all = [F.elu(x) for x in x_all]
            x_all = [F.dropout(input=F.normalize(x, dim=-1), p=self.dropout, training=self.training) for x in x_all]
        return x_all, torch.stack(ixz).mean(), torch.stack(structure_kl_loss).mean(), torch.stack(consensual).mean()